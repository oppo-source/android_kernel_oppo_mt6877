...
static const char *prjs[] = {
	"22803", "22881", "21001", "21201", "22091",
	"22227", "22101", "22235", "22236", NULL,
};

#define M2N(SZ) ((SZ) / HPAGE_CONT_PTE_SIZE)
#define HIGH_ORDER_GFP ((GFP_HIGHUSER | __GFP_ZERO | __GFP_NOWARN \
			 | __GFP_NORETRY) & ~__GFP_RECLAIM & ~__GFP_MOVABLE)
#define CONT_PTE_SUP_MEM_SIZE (6ul * SZ_1G)
...
atomic64_t cont_pte_double_map_count;
...
void count_vm_chp_event(enum vm_chp_event_item item)
{
...
void mod_chp_page_state(struct page *page, long delta)
{
...
bool is_thp_swap(struct swap_info_struct *si)
{
	return si && si->prio == THP_SWAP_PRIO_MAGIC;
}

bool is_cont_pte_cma(struct cma *cma)
{
	return cma == cont_pte_cma;
}

bool within_cont_pte_cma(unsigned long pfn)
{
...
bool transhuge_cont_pte_vma_suitable(struct vm_area_struct *vma,
						   unsigned long haddr)
{
	if (!vma_is_anonymous(vma)) {
		...
		if (!vma_is_chp_anonymous(vma))
			return false;
#endif
		if (unlikely(test_thread_flag(TIF_32BIT)))
			return false;
...
bool chp_handle_prctl_user_addrs(const char __user *name, unsigned long start,
				 unsigned long len)
{
	int i;
				 ...
				 bool freeze, struct page *page)
{
	...
	mmu_notifier_invalidate_range_start(
        vma->vm_mm, haddr, haddr + HPAGE_CONT_PTE_SIZE);
...
static inline void check_chp_supportable(char *p)
{
	int i = 0;
