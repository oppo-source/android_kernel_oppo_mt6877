...
static void save_unlock_password(struct mmc_card* card,u8* key_buffer,int key_len)
{
    ...
    memset(sdlock->unlock_pwd,0,MAX_UNLOCK_PASSWORD_WITH_BUF);
    sdlock->unlock_pwd[0] = (u8)key_len;
    memcpy(sdlock->unlock_pwd + 1 , key_buffer , key_len);
}
...
static int sd_erase_reinit(struct mmc_card* card , struct device* dev)
{
        int ret;
        int err = MMC_SUCCESS;
        typedef int (* sd_lock_reset)(struct mmc_host *host);
        ...
        if (card->host->ops->android_kabi_reserved1) {
                sd_lock_reset sd_lock_reset_fn = (sd_lock_reset)card->host->ops->android_kabi_reserved1;
                sd_lock_reset_fn(card->host);
        }
                   ...
                   const char *data, size_t len)
{
        ...
        if (mmc_card_locked(card) && !strncmp(command, "erase", 5)) {
                ...
                if(MMC_SUCCESS==ret) {
			printk("[SDLOCK] %s unlock successfully!\n",__func__);
                        ret = len;
                }
