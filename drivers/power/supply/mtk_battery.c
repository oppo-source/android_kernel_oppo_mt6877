...
void oplus_startup_rm_check_work_handler(struct work_struct *data)
{
	...
	if ((oppo_gm->prev_batt_remaining_capacity == 0) && (retry_times++ < RM_CHECK_RETRY_TIMES))
	{
		wakeup_fg_algo(oppo_gm, FG_INTR_IAVG);
		schedule_delayed_work(&oppo_gm->oplus_startup_rm_check_work, RM_CHECK_DELAY_20S);
	}
}
#endif
...
int battery_init(struct platform_device *pdev)
{
	...
	INIT_DELAYED_WORK(&oppo_gm->oplus_startup_rm_check_work, oplus_startup_rm_check_work_handler);
	schedule_delayed_work(&oppo_gm->oplus_startup_rm_check_work, RM_CHECK_DELAY_20S);
#endif
