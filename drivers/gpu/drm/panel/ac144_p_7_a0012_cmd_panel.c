...
static void oplus_lcm_panel_get_data(struct lcm *ctx)
{
	...
	DISP_INFO("return Page00_0E %d data(0x%08x) to dsi engine\n",ret, buffer[0]);
}
...
void lcm_get_info_p7(struct drm_panel *panel, int read_ic)
{
	...
	if (IS_ERR(ctx->reset_gpio))
		...
		DISP_INFO("%s, reset_gpio = %d\n", __func__, ret);
	}

	/* VCI LDO GPIO enable */
	reg_val = pmic_dio_get_ldo_6_enable_reg();
	pr_info("%s, ldo6 enable reg = 0x%08x\n", __func__, reg_val);
	if (reg_val < 0) {
		DISP_ERR("%s, get LDO6 failed\n", __func__);
	}
	reg_val = pmic_dio_get_ldo_6_value_reg();
	pr_info("%s, ldo6 value reg = %d\n", __func__, reg_val);
	if (reg_val < 0) {
		DISP_ERR("%s, get LDO6 failed\n", __func__);
	}
