...
static int hbm_brightness_revovery = 0;
...
static int panel_osc_mipi_freq_change(void *dsi, dcs_write_gce cb, void *handle, unsigned int en)
{
	...
	char osc_tb7[] = {0xC5, 0x32, 0x4C};	// OSC=167MHz@MIPI Speed=1.7Gbps
	char osc_tb8[] = {0xF0, 0xA5, 0xA5};
	char osc_tb9[] = {0xFC, 0xA5, 0xA5};

	printk("debug for %s, %d\n", __func__, en);

	if (en == 0) {            // OSC=167MHz@MIPI Speed=1.7Gbps
		osc_tb7[1] = 0x32;
		osc_tb7[2] = 0x4C;
	} else if (en == 1) {     // OSC=169MHz@MIPI Speed=1.746Gbps
		osc_tb7[1] = 0x31;
		osc_tb7[2] = 0x8F;
	} else if (en == 2) {     // OSC=169MHz@MIPI Speed=1.7Gbps
		osc_tb7[1] = 0x32;
		osc_tb7[2] = 0xE6;
	} else if (en == 3) {     // OSC=167MHz@MIPI Speed=1.746Gbps
		osc_tb7[1] = 0x30;
		osc_tb7[2] = 0xF9;
	}
...
static void lcm_panel_init(struct lcm *ctx)
{
	...
	if (osc_mipi_hopping_status == 0) {
		lcm_dcs_write_seq_static(ctx, 0xC5, 0x32, 0x4C);    // OSC=167MHz@MIPI Speed=1.7Gbps
	} else if (osc_mipi_hopping_status == 1) {
		lcm_dcs_write_seq_static(ctx, 0xC5, 0x31, 0x8F);    // OSC=169MHz@MIPI Speed=1.746Gbps
	} else if (osc_mipi_hopping_status == 2) {
		lcm_dcs_write_seq_static(ctx, 0xC5, 0x32, 0xE6);    // OSC=169MHz@MIPI Speed=1.7Gbps
	} else if (osc_mipi_hopping_status == 3) {
		lcm_dcs_write_seq_static(ctx, 0xC5, 0x30, 0xF9);    // OSC=167MHz@MIPI Speed=1.746Gbps
	}
	lcm_dcs_write_seq_static(ctx, 0xF0, 0xA5, 0xA5);
		...
		void *handle, unsigned int level)
{
	...
	if (level > 2047 && last_backlight_level < 2047 && last_backlight_level > 12) {
		if ((level - last_backlight_level) > 1000 && !hbm_brightness_revovery) {
			level = last_backlight_level;
		} else {
			hbm_brightness_revovery = 0;
			pr_info("debug for lcm hbm brightness revovery, level = %d \n", level);
		}
	}
