...
#include "../mediatek/mtk_corner_pattern/oplus22610_AMS643AG01_data_hw_roundedpattern.h"
#endif
...
static unsigned int osc_mipi_hopping_status;
 ...
 */
static int panel_osc_mipi_freq_change(void *dsi, dcs_write_gce cb, void *handle, unsigned int en)
{
	...
	char osc_tb7[] = {0xC5, 0x33, 0x33};	// OSC=170MHz@MIPI Speed=1.7Gbps
	...
	if (en == 0) {
		osc_tb7[1] = 0x33, osc_tb7[2] = 0x33;    // OSC=170MHz@MIPI Speed=1.7Gbps
	} else if (en == 1) {
		osc_tb7[1] = 0x32, osc_tb7[2] = 0x70;    // OSC=172MHz@MIPI Speed=1.746Gbps
	} else if (en == 2) {
		osc_tb7[1] = 0x33, osc_tb7[2] = 0xCD;    // OSC=172MHz@MIPI Speed=1.7Gbps
	} else if (en == 3) {
		osc_tb7[1] = 0x31, osc_tb7[2] = 0xDA;    // OSC=170MHz@MIPI Speed=1.746Gbps
	}
...
static void lcm_panel_init(struct lcm *ctx)
{
	...
	lcm_dcs_write_seq_static(ctx, 0xB0, 0x66, 0xC5);
	lcm_dcs_write_seq_static(ctx, 0xC5, 0x00, 0x8C);
	lcm_dcs_write_seq_static(ctx, 0xB0, 0x2A, 0xC5);
	lcm_dcs_write_seq_static(ctx, 0xC5, 0x0D, 0x10, 0x80, 0x45);
	lcm_dcs_write_seq_static(ctx, 0xB0, 0x3E, 0xC5);
	if (osc_mipi_hopping_status == 0) {
		lcm_dcs_write_seq_static(ctx, 0xC5, 0x33, 0x33);    // OSC=170MHz@MIPI Speed=1.7Gbps
	} else if (osc_mipi_hopping_status == 1) {
		lcm_dcs_write_seq_static(ctx, 0xC5, 0x32, 0x70);    // OSC=172MHz@MIPI Speed=1.746Gbps
	} else if (osc_mipi_hopping_status == 2) {
		lcm_dcs_write_seq_static(ctx, 0xC5, 0x33, 0xCD);    // OSC=172MHz@MIPI Speed=1.7Gbps
	} else if (osc_mipi_hopping_status == 3) {
		lcm_dcs_write_seq_static(ctx, 0xC5, 0x31, 0xDA);    // OSC=170MHz@MIPI Speed=1.746Gbps
	}
	lcm_dcs_write_seq_static(ctx, 0xF0, 0xA5, 0xA5);
		...
		unsigned int dst_mode, enum MTK_PANEL_MODE_SWITCH_STAGE stage)
{
	...
	pr_err("debug for lcm %s, dst_mode = %d, cur_mode = %d\n", __func__, dst_mode, cur_mode);
	pr_err("debug for lcm %s, stage = %d\n", __func__, stage);
		...
		void *handle, unsigned int level)
{
	...
	else if((first_backlight_level == 1) && (level == 4095)) {
		level = 1221;
	...
	pr_err("debug for lcm %s, level = %d, mapped_level = %d\n",
...
//	.doze_enable_start = panel_doze_enable_start,
	...
	.lcm_osc_mipi_change = panel_osc_mipi_freq_change,
