...
extern int pmic_ldo_get_register(unsigned int ldo_num, int *get_mv);
...
static int vddr6_buck4_regulator_get_voltage(struct device *dev)
{
	...
	if (!IS_ERR_OR_NULL(mt6315_6_vbuck4)) {
		...
		if (ret < 0)
			DISP_ERR("get voltage mt6315_6_vbuck4 fail, ret = %d\n", ret);
		retval |= ret;
	}

	DISP_INFO("get vddr6_buck4_regulator_get_voltage = %d\n", retval);
	return retval;
}
...
static int lcm_panel_poweron(struct drm_panel *panel)
{
	...
	usleep_range(2000, 2100);
	/* VCI LDO enable */
	ret = pmic_ldo_set_voltage_mv(3,3000);
	if (ret < 0) {
		DISP_INFO("debug for pmic_oled ldo3 set 3000 fail\n");
	} else {
		DISP_INFO("debug for pmic_oled ldo3 set 3000 success\n");
	}
...
static void oplus_lcm_panel_get_data(struct lcm *ctx)
{
	...
	lcm_dcs_write_seq_static(ctx, 0xFF, 0x08, 0x38, 0x1c);
	ret = oplus_lcm_dcs_read(ctx, 0xD7, buffer, 1);
	/* D7: bit1 0=vci off  1=on, bit0 0=vddi off 1=on */
	DISP_INFO("0xD7:data(0x%08x) to dsi engine\n",ret, buffer[0] | (buffer[1] << 8));
	memset(buffer, 0, sizeof(buffer));
	ret = oplus_lcm_dcs_read(ctx, 0xD6, buffer, 1);
	/* D6: bit7 0=avdd lvd off 1=on, bit0 0=elvdd lvd off 1=on */
	DISP_INFO("0xD6:data(0x%08x) to dsi engine\n",ret, buffer[0] | (buffer[1] << 8));
	/* B2: bit1 Detect three voltage status registers */
	lcm_dcs_write_seq_static(ctx, 0xFF, 0x08, 0x38, 0x20);
	memset(buffer, 0, sizeof(buffer));
	ret = oplus_lcm_dcs_read(ctx, 0xB2, buffer, 1);
	DISP_INFO("0xB2:data(0x%08x) to dsi engine\n",ret, buffer[0] | (buffer[1] << 8));

	lcm_dcs_write_seq_static(ctx, 0xFF, 0x08, 0x38, 0x00);
	memset(buffer, 0, sizeof(buffer));
	ret = oplus_lcm_dcs_read(ctx, 0x0A, buffer, 1);
	DISP_INFO("0x0A:data(0x%08x) to dsi engine\n",ret, buffer[0] | (buffer[1] << 8));
	/* 0E: bit7 0=te off, 1=te on */
	memset(buffer, 0, sizeof(buffer));
	ret = oplus_lcm_dcs_read(ctx, 0x0E, buffer, 1);
	DISP_INFO("0x0E:data(0x%08x) to dsi engine\n",ret, buffer[0] | (buffer[1] << 8));
	/* 0x80 = ddi sleep out */
	memset(buffer, 0, sizeof(buffer));
	ret = oplus_lcm_dcs_read(ctx, 0x09, buffer, 1);
	DISP_INFO("0x09:data(0x%08x) to dsi engine\n",ret, buffer[0] | (buffer[1] << 8));
	/* mipi err 0x05:0=No error, 1=Error */
	memset(buffer, 0, sizeof(buffer));
	ret = oplus_lcm_dcs_read(ctx, 0x05, buffer, 1);
	DISP_INFO("0x05:data(0x%08x) to dsi engine\n",ret, buffer[0] | (buffer[1] << 8));
}

#define VCI_LDO3 (3)
#define WL2868C_LDOx_EN (0x0e)
void oplus_lcm_read_info(struct drm_panel *panel, int read_ic)
{
	unsigned int ret = 0;
	int ldo_reg_val = 0;
	struct lcm *ctx = panel_to_lcm(panel);
	...
	/* LDO3 3v get voltage */
	/* VOUTx=1.504V+LDOx_VOUT [7:0]*0.008. Program range: 1.504V~3.544V, default VOUT3x=2.8V*/
	ret = pmic_ldo_get_register(VCI_LDO3, &ldo_reg_val);
	if (ret < 0) {
		DISP_ERR(" LDO3 3v get LDOx_VOUT fail!\n");
	} else {
		DISP_INFO(" LDO3 3v get LDOx_VOUT = %d\n", ldo_reg_val);
	}

	/* WL2868C reg:0x0e  Enable signal*/
	ret = pmic_ldo_get_register(WL2868C_LDOx_EN, &ldo_reg_val);
	if (ret < 0) {
		DISP_ERR(" get WL2868C_LDOX_EN fail!\n");
	} else {
		DISP_INFO(" get WL2868C_LDOX_EN = 0x%08x\n", ldo_reg_val);
	}
...
static struct mtk_panel_funcs ext_funcs = {
	...
	.oplus_get_info = oplus_lcm_read_info,
};
#endif
