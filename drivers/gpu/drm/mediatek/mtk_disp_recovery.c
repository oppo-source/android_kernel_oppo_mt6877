...
#define OPLUS_ESD_CHECK_PERIOD 3000 /* ms */
...
#ifdef OPLUS_FEATURE_DISPLAY
        if (((prj_id == 22277) || (prj_id == 23231) || (prj_id == 23051) || (prj_id == 23241) || (prj_id == 23243)) && (get_eng_version() == AGING)) {
			DDPPR_ERR("%s, esd only check eint\n", __func__);
            esd_ctx->chk_mode = READ_EINT;
        }
#endif
...
static int mtk_drm_esd_check_worker_kthread(void *data)
{
	...
	while (1) {
		...
		//if (ret < 0) {
		//#else
		if (ret < 0 || ffl_backlight_backup == 0 || ffl_backlight_backup == 1 || (ret == 0 && !atomic_read(&esd_ctx->check_wakeup)) || power_mode != 2) {
		//#endif
