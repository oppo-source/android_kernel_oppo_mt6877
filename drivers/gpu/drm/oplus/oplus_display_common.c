...
int oplus_display_panel_get_oplus_max_brightness(void *buf)
{
	...
	(*brightness) = oplus_max_normal_brightness;
	printk(KERN_ERR "%s, oplus_max_normal_brightness = %d\n", __func__, oplus_max_normal_brightness);

	if (oplus_max_normal_brightness == 0) {
		(*brightness) = 2047;
		printk(KERN_ERR "%s, use 2047 as default oplus_max_normal_brightness\n", __func__);
	}
