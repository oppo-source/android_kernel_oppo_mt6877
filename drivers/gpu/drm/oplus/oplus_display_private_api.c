...
uint64_t panel_reg = 0x0;
uint64_t panel_reg_value = 0x0;
...
static ssize_t oplus_display_set_mipi_hopping(struct device *dev,
	...
	mtk_crtc_osc_freq_switch(crtc, osc_mode, 0);
		...
		struct device_attribute *attr, char *buf)
{
	...
	printk(KERN_INFO "%s - will read reg 0x%02x\n", __func__,  panel_reg);
	comp->funcs->io_cmd(comp, NULL, DSI_READ, para);

	printk("%s para[0]:0x%llx, para[1]:0x%llx, para[2]:0x%llx, para[3]:0x%llx, para[4]:0x%llx, para[5]:0x%llx!\n",
		__func__,
		para[0],
		para[1],
		para[2],
	...
	return sprintf(buf, "read reg:0x%02x, \n	result value:\n	para[0] = 0x%x, \n\
	para[1] = 0x%llx, \n	para[2] = 0x%llx, \n	para[3] = 0x%llx, \n\
	para[4] = 0x%llx.\n", panel_reg, para[0], para[1], para[2], para[3], para[4]);
}
...
static ssize_t oplus_display_set_panel_reg(struct device *dev,
	...
	ret = kstrtouint(buf, 16, &temp_save);
	panel_reg = temp_save;
	printk(KERN_INFO "%s - panel_reg = 0x%02x\n", __func__, panel_reg);
...
static DEVICE_ATTR(panel_reg, S_IRUGO|S_IWUSR, oplus_display_get_panel_reg, oplus_display_set_panel_reg);
