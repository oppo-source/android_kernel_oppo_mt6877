...
#elif IS_ENABLED(CONFIG_FB_NOTIFY)
#include <linux/fb.h>
#endif
...
#if IS_ENABLED(CONFIG_DRM_MSM) || IS_ENABLED(CONFIG_DRM_OPLUS_NOTIFY) || IS_ENABLED(CONFIG_FB_NOTIFY)
...
#define CHP_OVER_COMPRESSED_RATIO 200 /* zram1 vs anon hugepages: 2 vs 1*/
...
static int swapd_init(struct zram **zram)
{
	...
	if (ret) {
		log_err("fb_register_client failed, ret=%d\n", ret);
		goto fb_notif_register_fail;
	}
#endif
