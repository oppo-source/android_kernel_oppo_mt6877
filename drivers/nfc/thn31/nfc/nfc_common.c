...
static int nfc_platform_clk_init(struct nfc_info *nfc)
{
    ...
    rc = clk_set_parent(nfc->clk, nfc->clk_parent);
    if (rc < 0) {
      pr_err("clk: failed to reparent clk to clk_parent: %d\n",rc);
    }
    ...
    rc = clk_prepare_enable(nfc->clk);
    if (rc) {
       pr_err("cannot force-on bringup nfc clk node\n");
    }

    rc = clk_prepare_enable(nfc->clk_enable);
    if (rc) {
       pr_err("cannot force-on bringup nfc clk_enable node\n");
    }
