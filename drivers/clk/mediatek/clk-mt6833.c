...
static struct clk_onecell_data *mt6833_top_clk_data;
...
static int clk_mt6833_top_probe(struct platform_device *pdev)
{
	...
	mtk_clk_register_muxes(top_muxes, ARRAY_SIZE(top_muxes), node,
			&mt6833_clk_lock, mt6833_top_clk_data);

	mtk_clk_register_composites(top_composites, ARRAY_SIZE(top_composites),
			base, &mt6833_clk_lock, mt6833_top_clk_data);
...
static int clk_mt6833_ifrao_probe(struct platform_device *pdev)
{
	struct clk_onecell_data *clk_data;
	int r;
	struct device_node *node = pdev->dev.of_node;
...
static int clk_mt6833_peri_probe(struct platform_device *pdev)
{
	struct clk_onecell_data *clk_data;
	int r;
	struct device_node *node = pdev->dev.of_node;
...
static const struct of_device_id of_match_clk_mt6833[] = {
	{
		...
		.data = clk_mt6833_ifrao_probe,
	}, {
		.compatible = "mediatek,mt6833-pericfg",
		.data = clk_mt6833_peri_probe,
	}, {
		/* sentinel */
	}
};
...
static int clk_mt6833_probe(struct platform_device *pdev)
{
	...
	r = clk_probe(pdev);
	if (r)
		pr_notice("could not register clock provider: %s: %d\n",
			pdev->name, r);
...
static int __init clk_mt6833_init(void)
{
