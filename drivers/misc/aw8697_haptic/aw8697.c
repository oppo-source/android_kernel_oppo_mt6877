...
#ifdef OPLUS_FEATURE_CHG_BASIC
/* Modify for using different rtp files by f0 */
...
static void aw8697_rtp_work_routine(struct work_struct *work)
{
	...
	int ret = -1;
	struct aw8697 *aw8697 =
		container_of(work, struct aw8697, rtp_work.work);

	pr_info("%s enter: device_id = %d, f0 = %d\n", __func__,
		aw8697->device_id, aw8697->f0);
				...
				size_t count)
{
	...
	if (val < (sizeof(aw8697_rtp_name) / AW8697_RTP_NAME_MAX)) {
		...
		if (val) {
			...
			queue_delayed_work(system_unbound_wq, &aw8697->rtp_work,
					   msecs_to_jiffies(audio_delay));
		}
	} else {
				    ...
				    struct device_attribute *attr, char *buf)
{
	...
	ssize_t len = 0;
	len += snprintf(buf + len, PAGE_SIZE - len,
			"aw8697 cont drv level = %u\n", aw8697->cont_drv_lvl);
	len += snprintf(buf + len, PAGE_SIZE - len,
			"aw8697 cont drv level overdrive= %u\n",
			aw8697->cont_drv_lvl_ov);
	return len;
}
...
/* for audio_delay for debug interface */
...
/* for audio_delay for debug interface */
static DEVICE_ATTR(audio_delay, S_IWUSR | S_IRUGO, aw8697_audio_delay_show,
...
static struct attribute *aw8697_vibrator_attributes[] = {
	...
	/* for audio_delay for debug interface */
	&dev_attr_audio_delay.attr, NULL
};
...
#ifdef OPLUS_FEATURE_CHG_BASIC
	/* Add for  vibration run in ram mode */
...
static int aw8697_vibrator_init(struct aw8697 *aw8697)
{
	...
	/* for audio_delay for audio & vibrator sync */
