...
extern bool oplus_get_vooc_direct_charge_en(void);
#endif
...
static inline void typec_handle_pd_wait_bc12(struct tcpc_device *tcpc)
{
...
#ifdef OPLUS_FEATURE_CHG_BASIC
	uint8_t wait_bc12_times = 20;
#endif
	mutex_lock(&tcpc->access_lock);
#ifdef OPLUS_FEATURE_CHG_BASIC
	if (tcpc->desc.pd_wait_bc12_times)
		wait_bc12_times = tcpc->desc.pd_wait_bc12_times;
#endif
	...
	if ((ret >= 0 && val.intval != POWER_SUPPLY_USB_TYPE_UNKNOWN) ||
		tcpc->pd_wait_bc12_count >= wait_bc12_times) {
#else
	if ((ret >= 0 && val.intval != POWER_SUPPLY_USB_TYPE_UNKNOWN) ||
...
#ifdef OPLUS_FEATURE_CHG_BASIC
	if ((tcpc->pd_during_direct_charge || oplus_get_vooc_direct_charge_en()) &&
