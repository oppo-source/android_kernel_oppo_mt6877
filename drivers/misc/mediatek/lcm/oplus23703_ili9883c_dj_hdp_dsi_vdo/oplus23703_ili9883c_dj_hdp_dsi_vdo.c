...
#include "oplus23703_ili9883c_dj_hdp_dsi_bl_map.h"
...
static void init_global_exp_backlight(void)
{
	...
	/*for (i=0; i<4096; i++) {
		LCM_LOGI("%s, ili9883c_djn backlight: map_exp[%d]=%d\n", __func__, i, map_exp[i]);
	}*/
...
static void lcm_dfps_int(struct LCM_DSI_PARAMS *dsi)
{
	...
	/* if vfp solution */
	dfps_params[0].vertical_frontporch = 1340;
	//dfps_params[0].vertical_frontporch_for_low_power = 2350; //45hz
	...
	//dfps_params[1].vertical_frontporch_for_low_power = 2350; //45hz
	...
	/* if vfp solution */
	dfps_params[2].vertical_frontporch = 1940;
	//dfps_params[2].vertical_frontporch_for_low_power = 2350; //45hz
...
static void lcm_get_params(struct LCM_PARAMS *params)
{
	...
	params->dsi.horizontal_frontporch_dyn = 112;
...
static void lcm_init_power(void)
{
	...
	MDELAY(2);
	LCM_LOGI("ili9883c_djn power_on\n");
}

static void lcm_suspend_power(void)
{
	if(0 == tp_gesture_enable_flag()) {
		LCM_LOGI("ili9883c_djn suspend_power\n");
...
static void lcm_init_set_cabc(int cabc_mode) {
	...
	LCM_LOGI("%s [ark] djn init cabc_mode is %d \n", __func__, cabc_mode);
...
{
	LCM_LOGI("ili9883c_djn shudown_power\n");
...
static unsigned int lcm_esd_recover(void)
{
	...
	LCM_LOGI("ili9883c_djn esd_recovery level = %d\n", level);
...
static void lcm_setbacklight_cmdq(void *handle, unsigned int level)
{
	...
	if (level == 0) {
		LCM_LOGI("ili9883c_djn backlight diming off\n");
	...
	LCM_LOGI("ili9883c_djn backlight: lvl=%d, level=%d, esd_last_level=%d\n",
...
static void dj_ili9883c_lcm_set_cabc_mode(void *handle, unsigned int mode)
{
	int mapped_level = 0;
...
struct LCM_DRIVER oplus23703_ili9883c_dj_hdp_dsi_vdo_drv = {
	...
	.update = lcm_update,
	.set_cabc_mode_cmdq = dj_ili9883c_lcm_set_cabc_mode,
};
