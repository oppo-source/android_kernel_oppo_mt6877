...
#include "lens_info.h"
#include "AW8601xAF_CHANEL_OV64B.h"

#define AW8601_SLAVE_ADDR		(0x18) /* 0x0c: 0x18 >> 1 */
 ...
 * I2c read/write
 ******************************************************************************/
static int AW8601_WriteRegs(unsigned char a_uAddr, unsigned char *a_uData,
							unsigned int len)
{
	int ret = 0;
	...
	g_pstAF_I2Cclient->addr = (AW8601_SLAVE_ADDR >> 1);
...
static int AW8601_ReadRegs(unsigned char a_uAddr, unsigned char *a_puData,
...
static inline int AW8601_SoftReset(void)
{
	uint8_t val = 0;
	int ret = 0;

	val = AW_SHUTDOWN;
	ret = AW8601_WriteRegs(AW_REG_CONTROL, &val, AW_DATA_BYTE_1);
	if (ret < 0) {
...
static inline int AW8601_CfgCurrentGears(unsigned char chip_id)
{
	uint8_t gears = 0;
	...
	if (chip_id == AW86017_CHIPID) {
		...
		AW8601_WriteRegs(AW_REG_IMAX, &gears, AW_DATA_BYTE_1);
	} else {
...
static inline int AW8601_SetAlgoMode(void)
{
	uint8_t mode_cmd[] = { 0xed, 0xab };

	uint8_t mode_cmd1[] = { 0x02, 0x02 };
	uint8_t mode_cmd2[] = { 0x06, 0x40 };
	uint8_t mode_cmd3[] = { 0x07, 0x08 };
	...
	/* enter advance mode */
	ret = AW8601_WriteRegs(mode_cmd[0], &mode_cmd[1], AW_DATA_BYTE_1);
	if (ret < 0) {
		AW_LOGE("Enter advance mode failed, ret: %d.", ret);
		return -EAGAIN;
	}

	ret = AW8601_WriteRegs(mode_cmd1[0], &mode_cmd1[1], AW_DATA_BYTE_1);
	if (ret < 0) {
		AW_LOGE("af setting init mode_cmd1, ret: %d.", ret);
		return -EAGAIN;
	}
	ret = AW8601_WriteRegs(mode_cmd2[0], &mode_cmd2[1], AW_DATA_BYTE_1);
	if (ret < 0) {
		AW_LOGE("af setting init mode_cmd2, ret: %d.", ret);
		return -EAGAIN;
	}
	ret = AW8601_WriteRegs(mode_cmd3[0], &mode_cmd3[1], AW_DATA_BYTE_1);
	if (ret < 0) {
		AW_LOGE("af setting init mode_cmd3, ret: %d.", ret);
		return -EAGAIN;
	}

	/* Identification IC */
	ret = AW8601_ReadRegs(AW_REG_CHIP_ID, &chip_id, AW_DATA_BYTE_1);
	if (ret < 0) {
	...
	if (type == AW_BOTTOM_MOTOR) {
		...
		ret = AW8601_WriteRegs(AW_REG_ALG_MODE, algo_cfg,
								AW_DATA_BYTE_3);
		if (ret < 0) {
		...
		ret = AW8601_ReadRegs(AW_REG_CONTROL, &ctl_val,
								AW_DATA_BYTE_1);
		if (ret < 0) {
		...
		ret = AW8601_WriteRegs(AW_REG_CONTROL, &ctl_val,
								AW_DATA_BYTE_1);
		if (ret < 0) {
	...
	AW8601_CfgCurrentGears(chip_id);
#endif
	g_motor_type = type;
...
static inline int AW8601_InitDrv(void)
{
	int ret = 0;
...
static int AW8601_SetPos(unsigned long a_u4Position)
{
	int ret = 0;
	...
	AW_LOGD("Start.");
 	while (loop_time < 20) {
          status = AW8601_ReadRegs(AW_REG_STATUS, &status_val, AW_DATA_BYTE_1);
	...
	AW_LOGI("Target Position = %d", a_u4Position);

	g_pstAF_I2Cclient->addr = (AW8601_SLAVE_ADDR >> 1); /* 0x18 >> 1 */
...
static inline int AW8601_MoveVCM(unsigned long a_u4Position)
{
	AW_LOGD("Start.");
	...
	if (*g_pAF_Opened == 2) {
		if (AW8601_SetPos(a_u4Position) == 0) {
			spin_lock(g_pAF_SpinLock);
...
static inline int AW8601_SetMacro(unsigned long a_u4Param)
{
	AW_LOGI("Start.");
...
static inline int AW8601_SetInf(unsigned long a_u4Param)
{
	AW_LOGI("Start.");
...
static inline int AW8601_GetVCMInfo(
...
long AW8601_CHANEL_OV64B_Ioctl(struct file *a_pstFile, unsigned int a_u4Command,
							unsigned long a_u4Param)
{
	...
	case AFIOC_G_MOTORINFO:
		ret = AW8601_GetVCMInfo(
				(__user struct stAF_MotorInfo *) (a_u4Param));
		break;
	case AFIOC_T_MOVETO:
		ret = AW8601_MoveVCM(a_u4Param);
		break;
	case AFIOC_T_SETMACROPOS:
		ret = AW8601_SetMacro(a_u4Param);
		break;
	case AFIOC_T_SETINFPOS:
		ret = AW8601_SetInf(a_u4Param);
		break;
	default:
		AW_LOGE("NO CMD!.");
		ret = -EPERM;
...
int AW8601_CHANEL_OV64B_Release(struct inode *a_pInode, struct file *a_pstFile)
{
	int ret = 0;
...
int AW8601_CHANEL_OV64B_PowerDown(struct i2c_client *pstAF_I2Cclient, int *pAF_Opened)
{
	...
	/* enter PD mode, PD = 1; */
	ret = AW8601_WriteRegs(pd_cmd[0], &pd_cmd[1], AW_DATA_BYTE_1);
	if (ret < 0) {
...
int AW8601_CHANEL_OV64B_SetI2Cclient(struct i2c_client *pstAF_I2Cclient,
...
int AW8601_CHANEL_OV64B_GetFileName(uint8_t *pFileName)
{
