...
bool oplus_display_no_support_mtk_round_corner;
bool oplus_display_mt6382_esd_check;
bool oplus_display_panel_cabc_support;
bool oplus_display_panel_cabc_power_saving_support;
bool oplus_display_bls_diversion_support;
#ifdef CONFIG_MTK_MT6382_BDG
bool oplus_display_async_bl_support;
unsigned int g_mininum_brightness = 14;
#endif
...
static int mtkfb_probe(struct platform_device *pdev)
{
	...
	oplus_display_panel_cabc_support = of_property_read_bool(pdev->dev.of_node, "oplus_display_panel_cabc_support");
	if (!oplus_display_panel_cabc_support)
		dev_err(&pdev->dev, "read bool oplus_display_panel_cabc_support failed.");
	else
		DISPMSG("%s:oplus_display_panel_cabc_support=%u\n", __func__, oplus_display_panel_cabc_support);

	oplus_display_panel_cabc_power_saving_support = of_property_read_bool(pdev->dev.of_node, "oplus_display_panel_cabc_power_saving_support");
	if (!oplus_display_panel_cabc_power_saving_support)
		dev_err(&pdev->dev, "read bool oplus_display_panel_cabc_power_saving_support failed.");
	else
		DISPMSG("%s:oplus_display_panel_cabc_power_saving_support=%u\n", __func__, oplus_display_panel_cabc_power_saving_support);

	oplus_display_bls_diversion_support = of_property_read_bool(pdev->dev.of_node, "oplus_display_bls_diversion_support");
	if (!oplus_display_bls_diversion_support)
		dev_err(&pdev->dev, "read bool oplus_display_bls_diversion_support failed.");
	else
		DISPMSG("%s:oplus_display_bls_diversion_support=%u\n", __func__, oplus_display_bls_diversion_support);

	oplus_display_no_support_mtk_round_corner = of_property_read_bool(pdev->dev.of_node, "oplus_display_no_support_mtk_round_corner");
	if (!oplus_display_no_support_mtk_round_corner)
		dev_err(&pdev->dev, "read bool oplus_display_no_support_mtk_round_corner failed.");
	else
		DISPMSG("%s:oplus_display_no_support_mtk_round_corner=%u\n", __func__, oplus_display_no_support_mtk_round_corner);

	oplus_display_mt6382_esd_check = of_property_read_bool(pdev->dev.of_node, "oplus_display_mt6382_esd_check");
	if (!oplus_display_mt6382_esd_check)
		dev_err(&pdev->dev, "read bool oplus_display_mt6382_esd_check failed.");
	else
		DISPMSG("%s:oplus_display_mt6382_esd_check=%u\n", __func__, oplus_display_mt6382_esd_check);

#ifdef CONFIG_MTK_MT6382_BDG
	oplus_display_async_bl_support = of_property_read_bool(pdev->dev.of_node, "oplus_display_async_bl_support");
	if (!oplus_display_async_bl_support)
		dev_err(&pdev->dev, "read bool oplus_display_async_bl_support failed.");
	else
		DISPMSG("%s:oplus_display_async_bl_support=%u\n", __func__, oplus_display_async_bl_support);

	of_ret = of_property_read_u32(pdev->dev.of_node, "mininum_brightness", &g_mininum_brightness);
	if (!of_ret) {
		g_mininum_brightness = 14;
		dev_err(&pdev->dev, "read mininum_brightness failed.");
	} else {
		DISPMSG("%s:mininum_brightness=%u\n", __func__, g_mininum_brightness);
	}
#endif
...
/*mt6382 mipi hopping clk register*/
	...
	if (!strcmp(mtkfb_find_lcm_driver(),"ilt9881h_txd_hdp_dsi_vdo_lcm_drv")
      ...
      ||!strcmp(mtkfb_find_lcm_driver(),"ac112_p_7_a0009_fhd_dsi_vdo_lcm_drv")
      ||!strcmp(mtkfb_find_lcm_driver(),"ac112_p_d_a0010_fhd_dsi_vdo_lcm_drv")
      ||!strcmp(mtkfb_find_lcm_driver(),"ac114_p_3_a0013_video_panel")
	  ||!strcmp(mtkfb_find_lcm_driver(),"oplus23703_ili9883c_dj_hdp_dsi_vdo")
	  ||!strcmp(mtkfb_find_lcm_driver(),"oplus23703_ili9883c_hkc_hdp_dsi_vdo")) {
		register_ccci_sys_call_back(MD_SYS1,
			MD_DISPLAY_DYNAMIC_MIPI, mipi_clk_change);
	}
#endif
