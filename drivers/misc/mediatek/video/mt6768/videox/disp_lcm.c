...
extern bool oplus_display_bls_diversion_support;
	...
	void *handle, int level)
{
	...
	if(lcm_params->blmap && (level != 0)) {
		level = lcm_params->blmap[level];
	}

	if (level < lcm_params->brightness_min && (bl_level_bak != 0)) {
		if(oplus_display_bls_diversion_support && lcm_params->backlight_diversion_map) {
			bl_diversion_level = lcm_params->backlight_diversion_map[bl_level_bak];
		}
	...
	if (lcm_drv->set_backlight_cmdq) {
		...
		DISPDBG("oplus_display_bls_diversion_support=%d,bl_level_bak=%d,level=%d,bl_diversion_level=%d\n",oplus_display_bls_diversion_support, bl_level_bak, level, bl_diversion_level);
		if (oplus_display_bls_diversion_support) {
			disp_bls_diversion_set_value(bl_diversion_level);
		}
	} else {
