...
int mipi_clk_change(int msg, int en)
{
	...
	if (en) {
		...
		} else if (!strcmp(mtkfb_lcm_name,"oplus22351_tm_nt36672c_fhd_dsi_vdo_lcm_drv")) {
			def_data_rate = 1086;
			if (dsi_params->mode != CMD_MODE)
				def_dsi_hbp = 0x0C;
				mipi_clk_change_sta = 1;
		} else if (!strcmp(mtkfb_lcm_name,"oplus22351_csot_ili7807s_fhd_dsi_vdo_lcm_drv")) {
			def_data_rate = 1086;
			if (dsi_params->mode != CMD_MODE)
				def_dsi_hbp = 0x0E;
				mipi_clk_change_sta = 1;
		} else if (!strcmp(mtkfb_lcm_name,"oplus22361_boe_ili9883c_hd_dphy_vdo_lcm_drv")) {
			def_data_rate = 914;
			if (dsi_params->mode != CMD_MODE)
				def_dsi_hbp = 0x18;
				mipi_clk_change_sta = 1;
	...
	if (_is_power_on_status(DISP_MODULE_DSI0)) {
		...
		/* 2.wait mutex0_stream_eof: only used for video mode */
			...
			if (bdg_is_bdg_connected() == 1) {
				...
				if (!strcmp(mtkfb_lcm_name,"oplus22351_tm_nt36672c_fhd_dsi_vdo_lcm_drv")
				||!strcmp(mtkfb_lcm_name,"oplus22351_csot_ili7807s_fhd_dsi_vdo_lcm_drv")) {
					DSI_MIPI_clk_change(DISP_MODULE_DSI0, handle, def_data_rate);
				}
				if (!strcmp(mtkfb_lcm_name,"oplus22361_boe_ili9883c_hd_dphy_vdo_lcm_drv")) {
					DSI_MIPI_clk_change(DISP_MODULE_DSI0, handle, def_data_rate);
				}
				ddp_dsi_porch_setting(DISP_MODULE_DSI0,
...
//	DDPMSG("%s +\n", __func__);
	...
	if (oplus_display_mt6382_support) {
		if (count > 1)
			cmdq_pkt_sleep_by_poll(cmdq->pkt, CMDQ_US_TO_TICK(100));
	}
#endif
	if (force_update) {
		DSI_Start(module, cmdq);
