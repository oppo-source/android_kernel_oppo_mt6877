...
static void read_EepromQSC(void)
{
    ...
    gImgEepromInfo_212A1.camNormdata[1][0] = Eeprom_1ByteDataRead(0x00, 0xA8);
    gImgEepromInfo_212A1.camNormdata[1][1] = Eeprom_1ByteDataRead(0x01, 0xA8);
    imgsensor_info.module_id = Eeprom_1ByteDataRead(0x00, 0xA8);
    Oplusimgsensor_Registdeviceinfo(gImgEepromInfo_212A1.pCamModuleInfo[1].name,
                                    gImgEepromInfo_212A1.pCamModuleInfo[1].version,
                                    imgsensor_info.module_id);
    for(i = 2; i < 8; i++) {
        gImgEepromInfo_212A1.camNormdata[1][i] = Eeprom_1ByteDataRead(0x04+i, 0xA8);
    }
    for (i = 0; i < OPPO_CAMERASN_LENS; i ++) {
        gImgEepromInfo_212A1.camNormdata[1][8+i] = Eeprom_1ByteDataRead(0xB0+i, 0xA8);
    }
    gImgEepromInfo_212A1.i4CurSensorIdx = 1;
    gImgEepromInfo_212A1.i4CurSensorId = imgsensor_info.sensor_id;
}
