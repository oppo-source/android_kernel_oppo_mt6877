...
#include "sc820cs_mipi_raw23618_Sensor.h"
#include "imgsensor_hwcfg_custom.h"

#define PFX "sc820cs_mipi_raw23618"
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = SC820CS_SENSOR_ID23618,
...
static struct SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[5] =
{
	...
	{ 3264, 2448, 0, 306,   3264, 1836, 3264, 1836, 0, 0, 3264, 1836,  0,  0, 3264, 1836},	/* hs video */
	{ 3264, 2448, 0, 306,   3264, 1836, 3264, 1836, 0, 0, 3264, 1836,  0,  0, 3264, 1836}};	/* slim video */
...
//add by wuyubin@v87596577 for sn 2024/1/3
...
#define OTP_RW_FLAG    0x0020
#define OTP_READ_FLAG  0x0021

#define MAX_GROUP_NUM_SN 2
#define PAGE_NUM 2
static int page_addr_start[PAGE_NUM] = {0x8200,0x8c00};
static int page_addr_end[PAGE_NUM] = {0x83ff,0x8dff};
static int page_addr[PAGE_NUM] = {0x03,0x0d};
static int sn_flag_addr_tbl[MAX_GROUP_NUM_SN] = {0x82BC, 0x8CBC};
static int sn_data_addr_tbl[MAX_GROUP_NUM_SN] = {0x82A4, 0x8CA4};

unsigned char sc820cs_data_sn[SN_DATA_SIZE+2] = {0};/*Add flag and check sum*/
unsigned char sc820cs_sn_valid = 0;

static bool sc820cs_sensor_otp_init(int group_idx)
{
	...
	*/
	write_cmos_sensor_8(0x36b0, 0x48);
	write_cmos_sensor_8(0x36b2, 0x41);
	write_cmos_sensor_8(0x4408, (page_addr_start[group_idx] >> 8)&0xff);        /* High byte start address for OTP load Group1 */
	write_cmos_sensor_8(0x4409, (page_addr_start[group_idx])&0xff);             /* Low byte start address for OTP load  Group1 */
	write_cmos_sensor_8(0x440a, (page_addr_end[group_idx] >> 8)&0xff);          /* High byte end address for OTP load Group1 */
	write_cmos_sensor_8(0x440b, (page_addr_end[group_idx])&0xff);              /* Low byte end address for OTP load Group1 */
	write_cmos_sensor_8(0x4401, 0x13);
	write_cmos_sensor_8(0x4412, page_addr[group_idx]);                                /*SN page_addr page2 page7*/
	write_cmos_sensor_8(0x4407, 0x00);
	write_cmos_sensor_8(0x4400, 0x11);                         /* 0x4400, 0x11 manual load */

	mdelay(20);
	return true;
}

static int read_sc820cs_sn_info(void)
{
	...
	for(i = 0; i < MAX_GROUP_NUM_SN; i++){
		write_cmos_sensor_8(0x4408, (sn_flag_addr_tbl[i]>>8)&0xff);
		write_cmos_sensor_8(0x4409, (sn_flag_addr_tbl[i])&0xff);
		write_cmos_sensor_8(OTP_RW_FLAG, 0x01);
		otp_grp_flag = read_cmos_sensor(sn_flag_addr_tbl[i]);
		if (otp_grp_flag == 0x01) {
			pr_info("sn: group %d selected\n", i);
			sn_start_addr = sn_data_addr_tbl[i];
			break;
		}
	}
	if(sn_start_addr != 0){
		write_cmos_sensor_8(0x4408,((sn_start_addr)>>8)&0xff);
		write_cmos_sensor_8(0x4409,(sn_start_addr)&0xff);
		write_cmos_sensor_8(OTP_RW_FLAG,0x01);
		for(i = 0; i < SN_DATA_SIZE+2 ; i++)  // add flag and checksum
		{
			sc820cs_data_sn[i] = read_cmos_sensor(sn_start_addr+i);
		}

		for(i = 0; i < SN_DATA_SIZE; i++){
			check_sum_sn_cal += sc820cs_data_sn[i];
		}
		pr_info("check_sum_sn_cal =0x%x \n",check_sum_sn_cal);
		check_sum_sn = sc820cs_data_sn[SN_DATA_SIZE+1];
		check_sum_sn_cal = check_sum_sn_cal % 255;

		pr_info("=== sc820cs SN check_sum_sn=0x%x, check_sum_sn_cal=0x%x ===\n", check_sum_sn, check_sum_sn_cal);
		if(check_sum_sn == check_sum_sn_cal){
			pr_info("check_sum_sn success!\n");
			for (i = 0; i < SN_DATA_SIZE; i++) {
				pCamDeviceObj->camNormdata[1][8+i] = (kal_uint8)sc820cs_data_sn[i];
			}
			pCamDeviceObj->pCamModuleInfo[1].i4Sensorid = imgsensor_info.sensor_id;
			pCamDeviceObj->i4CurSensorIdx = 1;
			pCamDeviceObj->i4CurSensorId = SC820CS_SENSOR_ID23618;
			return 1;
		}else{
			return 0;
			pr_err("check_sum_sn fail!\n");
		}
	} else {
		return 0;
	}
...
static bool sc820cs_sn_get(){
	int ret = 0,i = 0;
	for(i ; i < PAGE_NUM;i++){
		ret = sc820cs_sensor_otp_init(i);
		if (!ret){
			pr_err("sc820cs_sensor_otp_init false");
			return FALSE;
		}
		ret = read_sc820cs_sn_info();
		if (!ret){
			pr_err("read_sc820cs_sn_info false");
			return FALSE;
		}
		return TRUE;
	}
...
write_cmos_sensor(0x369a,0x8f);
write_cmos_sensor(0x369b,0xa1);
...
static void normal_video_setting(void)
{
    ...
    write_cmos_sensor(0x3203,0x32);
    write_cmos_sensor(0x3204,0x0c);
    write_cmos_sensor(0x3205,0xc7);
    write_cmos_sensor(0x3206,0x08);
    write_cmos_sensor(0x3207,0x6d);
    write_cmos_sensor(0x3208,0x0c);
    write_cmos_sensor(0x3209,0xc0);
    write_cmos_sensor(0x320a,0x07);
    write_cmos_sensor(0x320b,0x2c);
    write_cmos_sensor(0x320c,0x06);
    write_cmos_sensor(0x320d,0xe0);
    write_cmos_sensor(0x320e,0x09);
    write_cmos_sensor(0x320f,0xc4);
