...
#include "aladdin_ov02b1bmipiraw_Sensor.h"
...
#define PFX "aladdin_ov02b1b_camera_sensor"
...
static  struct imgsensor_info_struct imgsensor_info = {
	...
	.sensor_id = ALADDIN_OV02B1B_SENSOR_ID,
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			write_cmos_sensor(0xfd, 0x00);
			*sensor_id = ((read_cmos_sensor(0x02) << 8) | read_cmos_sensor(0x03) + 15);
			LOG_INF("aladdin_ov02b1b get_imgsensor_id:0x%x  i2c:0x%x\n", *sensor_id, imgsensor.i2c_write_id);
