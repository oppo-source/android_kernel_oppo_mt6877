...
#define PFX "miami_sc800cs_camera_sensor"
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = MIAMI_SC800CS_SENSOR_ID,
...
static struct SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[5] ={
	...
	{ 3264, 2448, 0, 0, 3264, 2448, 3264, 2448, 0, 0, 3264, 2448, 0, 0, 3264, 2448},
	/* video */
	{ 3264, 2448, 96, 360, 3072, 1728, 3072, 1728, 0, 0, 3072, 1728, 0, 0, 3072, 1728},
	/* hs video */
	{ 3264, 2448, 320, 256, 2624, 1936, 656, 484, 8, 2, 640, 480, 0, 0, 640, 480},
	/* slim video */
	{ 3264, 2448, 0, 0, 3264, 2448, 1632, 1224, 0, 0, 1632, 1224, 0, 0, 1632, 1224}
};

/**/
static void read_EepromSn(void)
{
	kal_uint16 i = 0, SC800CS_CAMERASN_LENS = 17;

	/*Read normal eeprom data*/
	gImgEepromInfo.camNormdata[1][0] = Eeprom_1ByteDataRead(0x00, 0xA2);
	gImgEepromInfo.camNormdata[1][1] = Eeprom_1ByteDataRead(0x01, 0xA2);
	LOG_INF("gImgEepromInfo.camNormdata[1][0]]:0x%x\n", gImgEepromInfo.camNormdata[1][0]);
	LOG_INF("gImgEepromInfo.camNormdata[1][1]:0x%x\n", gImgEepromInfo.camNormdata[1][1]);
	...
	for(i = 2; i < 8; i++) {
	    gImgEepromInfo.camNormdata[1][i] = Eeprom_1ByteDataRead(0x04+i, 0xA2);
	    LOG_INF("gImgEepromInfo.camNormdata[1][%d]:0x%x\n",i, gImgEepromInfo.camNormdata[1][i]);
	}
	for (i = 0; i < SC800CS_CAMERASN_LENS; i ++) {
	   gImgEepromInfo.camNormdata[1][8+i] = Eeprom_1ByteDataRead(0x0774+i, 0xA2);
	   LOG_INF("gImgEepromInfo.camNormdata[1][%d]:0x%x\n",8+i, gImgEepromInfo.camNormdata[1][8+i]);
	}
...
static void preview_setting(void)
{
	...
	write_cmos_sensor8(0x3650,0x31);//mipi driver capability default 0x3650,0x31
	write_cmos_sensor8(0x3651,0x9f);//mipi driver capability default 0x3651,0x9d
...
static void normal_video_setting(void)
{
	...
	write_cmos_sensor8(0x4002,0x66);
	write_cmos_sensor8(0x4003,0x66);
	write_cmos_sensor8(0x4004,0x66);
	write_cmos_sensor8(0x4005,0x00);
...
static void hs_video_setting(void)
{
	...
	write_cmos_sensor8(0x3e01,0x40);
	...
	write_cmos_sensor8(0x4005,0x00);
	write_cmos_sensor8(0x4006,0x03);
	write_cmos_sensor8(0x4007,0xee);
	write_cmos_sensor8(0x4008,0x00);
	write_cmos_sensor8(0x4009,0xfa);
	write_cmos_sensor8(0x440e,0x02);
	write_cmos_sensor8(0x4509,0x30);
	write_cmos_sensor8(0x4837,0x23);
...
static void slim_video_setting(void)
{
	...
	write_cmos_sensor8(0x36e9,0x00);
	write_cmos_sensor8(0x36f9,0x24);
	write_cmos_sensor8(0x301f,0x1d);
	write_cmos_sensor8(0x302d,0x20);
	write_cmos_sensor8(0x3106,0x01);
...
static kal_uint32 set_test_pattern_mode(kal_bool enable)
{
	...
	if (enable) {
		...
		write_cmos_sensor8(0x0100, 0x00);
		write_cmos_sensor8(0x4501, 0xa4);
		write_cmos_sensor8(0x3902, 0xc5);
		write_cmos_sensor8(0x3908, 0x41);
				...
				*sensor_id =return_sensor_id();
				...
				if (*sensor_id == imgsensor_info.sensor_id) {
					...
					if(deviceInfo_register_value == 0x00){
							Eeprom_DataInit(1, MIAMI_SC800CS_SENSOR_ID);
