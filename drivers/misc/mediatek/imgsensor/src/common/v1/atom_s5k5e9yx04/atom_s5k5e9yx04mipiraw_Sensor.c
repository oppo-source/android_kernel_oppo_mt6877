	...
	while(imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = return_sensor_id();
			printk("atom s5k5e9yxmipiraw_Sensor get_imgsensor_id *sensor_id = 0x%x",*sensor_id);
			if (*sensor_id == ATOM_S5K5E9YX04_SENSOR_ID) {
				printk("atom i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
				#ifdef OPLUS_FEATURE_CAMERA_COMMON
			...
			return ERROR_NONE;
			printk("atom Read sensor id fail, id: 0x%x\n", *sensor_id);
			retry--;
		} while(retry > 0);
		i++;
		retry = 2;
