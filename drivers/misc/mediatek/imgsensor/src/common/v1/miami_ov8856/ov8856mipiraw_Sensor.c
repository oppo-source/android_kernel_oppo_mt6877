...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = MIAMI_OV8856_SENSOR_ID,
...
{
	kal_uint16 i = 0, OV8856_CAMERASN_LENS = 17;
	...
	LOG_INF("gImgEepromInfo.camNormdata[2][0]]:0x%x\n", gImgEepromInfo.camNormdata[2][0]);
	LOG_INF("gImgEepromInfo.camNormdata[2][1]:0x%x\n", gImgEepromInfo.camNormdata[2][1]);
	...
	/*lens id*/
	gImgEepromInfo.camNormdata[2][4] = Eeprom_1ByteDataRead(0x03, 0xA2);
	LOG_INF("gImgEepromInfo.camNormdata[2][4]]:0x%x\n", gImgEepromInfo.camNormdata[2][4]);
	LOG_INF("gImgEepromInfo.camNormdata[2][5]]:0x%x\n", gImgEepromInfo.camNormdata[2][5]);
	/*qrcode*/
	for (i = 0; i < OV8856_CAMERASN_LENS; i++) {
	   gImgEepromInfo.camNormdata[2][8+i] = Eeprom_1ByteDataRead(0x09+i, 0xA2);
	   LOG_INF("gImgEepromInfo.camNormdata[1][%d]:0x%x\n",8+i, gImgEepromInfo.camNormdata[2][8+i]);
	}
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		spin_unlock(&imgsensor_drv_lock);
		do {
			LOG_INF("ov8856 get_imgsenor_id: 0x%x\n", *sensor_id);
			*sensor_id =
