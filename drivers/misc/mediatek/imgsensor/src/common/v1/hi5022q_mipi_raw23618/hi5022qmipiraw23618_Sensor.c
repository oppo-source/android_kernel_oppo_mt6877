...
/*hs14 code for AL6528ADEU-1243 by jianghongyan at 2022-10-28 start*/
...
#include "hi5022qmipiraw23618_Sensor.h"
#include "imgsensor_hwcfg_custom.h"

#define PFX "hi5022qmipiraw23618"
...
static kal_uint8 XGCbuffer[hi5022q_OTP_XGC_LEN+1] = {0};
static kal_uint8 QGCbuffer[hi5022q_OTP_QGC_LEN+1] = {0};
static kal_uint8 OPCbuffer[hi5022q_EEPROM_OPC_LEN_WAY1+1] = {0};
...
static struct imgsensor_info_struct imgsensor_info = {
     .sensor_id = HI5022Q_SENSOR_ID23618,
     ...
     .normal_video = { //PD2VC
         ...
         /*     following for GetDefaultFramerateByScenario()    */
         .max_framerate = 300,
         .mipi_pixel_rate =796800000,//1992Mbps*4/10
     },
     .hs_video = {
         .pclk = 124000000,
         .linelength =  464,
     ...
     .custom1 = {
         ...
         /*     following for GetDefaultFramerateByScenario()    */
         .max_framerate = 300,
         .mipi_pixel_rate =662400000,//1080Mbps*4/10
     },
     .custom2 = {
         .pclk = 124000000,
         .linelength =  1158,
     ...
     .custom4 = {
         ...
         /*     following for GetDefaultFramerateByScenario()    */
         .max_framerate = 300,
         .mipi_pixel_rate =598400000,//1656Mbps*4/10
     },
...
static struct SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[9] = {
     { 8224, 6176,  16,   16, 8192, 6144,  4096, 3072,   0,  0, 4096, 3072, 0, 0, 4096, 3072},  //preview(4080x3072)@30fps
     { 8224, 6176,  16,   16, 8192, 6144,  4096, 3072,   0,  0, 4096, 3072, 0, 0, 4096, 3072},  //capture(4080x3072)@30fps
     {8224, 6176,  16,  784, 8192, 4608,  4096, 2304,   0,  0, 4096, 2304, 0, 0, 4096, 2304},  //normal_video(4096x2304)@30fps
     {8224, 6176, 16,  928, 8192, 4320, 2048, 1080,   64, 0,  1920, 1080, 0,0, 1920, 1080},  //hs_video(1080P)@60fps
     {8224, 6176,  16,  928, 8192, 4320,  2048, 1080,   64, 0,  1920, 1080, 0,0, 1920, 1080},  //slim video(2048X1160)@120fps
     {8224, 6176, 16,  928, 8192, 4320, 4096, 2160,  128, 0,  3840, 2160, 0,0, 3840, 2160},  //custom1(3840X2160)@30fps
     {8224, 6176, 16,  928, 8192, 4320, 4096, 2160,  128, 0,  3840, 2160, 0,0, 3840, 2160},  //custom2(3840X2160)@24fps
     {8224, 6176, 16,   16, 8192, 6144, 8192, 6144,    0, 0,  8192, 6144, 0,0, 8192, 6144},  //custom3(8160x6144)@30fps
	 { 8224, 6176,  16,   16, 8192, 6144,  4096, 3072,   0,  0, 4096, 3072, 0, 0, 4096, 3072},	//custom4(4080x3072)@10fps
};
...
#define HI5022Q_EEPROM_SLAVE_ADDR 0XA1
...
static void read_4cell_from_eeprom(char *data)
{
    ...
    if (data != NULL)
    {
        ...
        memcpy(data+2, &OPCbuffer, hi5022q_EEPROM_OPC_LEN_WAY1);  //opc data
        ////new remosaic lib 1.3 need xgc also to lib
        memcpy(data+2+hi5022q_EEPROM_OPC_LEN_WAY1, &XGCbuffer, hi5022q_OTP_XGC_LEN);  //opc data
    }
    for(i=0;i< 16;i++)
    {
...
#define HI5022_ARK_EEPROM_I2C_ADDR 0xA0
static BYTE ark_common_data[CAMERA_EEPPROM_COMDATA_LENGTH] = { 0 };

static kal_uint16 read_ark_eeprom_module(kal_uint32 addr)
{
	kal_uint16 get_byte = 0;
	char pusendcmd[2] = { (char)(addr >> 8), (char)(addr & 0xFF) };

	iReadRegI2C(pusendcmd, 2, (u8 *) &get_byte, 1, HI5022_ARK_EEPROM_I2C_ADDR);

	return get_byte;
}
static void read_ark_module_data()
{
    kal_uint16 idx = 0;
    kal_uint16 sn_length = 24;
    kal_uint32 sn_starAddr = 0x58;
    int check_sum_sn_cal = 0;
    int check_sum_sn = 0;
    struct CAMERA_DEVICE_INFO *pCamDeviceObj = &gImgEepromInfo;
    memset(ark_common_data, 0,sizeof(ark_common_data));
    for(idx = 0; idx < sn_length+2; idx++)
    {
        ark_common_data[idx] = read_ark_eeprom_module(sn_starAddr + idx);
    }
    for(idx = 0;idx < sn_length + 1 ;idx++)
    {
        check_sum_sn_cal += ark_common_data[idx];
    }
    pr_info("check_sum_sn_cal=0x%x",check_sum_sn_cal);
    check_sum_sn = ark_common_data[sn_length+1];
    check_sum_sn_cal = check_sum_sn_cal % 255;
    pr_info("=== hi5022 SN check_sum_sn=0x%x, check_sum_sn_cal=0x%x ===\n", check_sum_sn, check_sum_sn_cal);
    if (check_sum_sn == check_sum_sn_cal){
        pr_info("check_sum_sn success!");
        for (idx = 0;idx < sn_length; idx++)
        {
            pCamDeviceObj->camNormdata[2][8+idx] = (kal_uint8)ark_common_data[idx];
        }
        pCamDeviceObj->pCamModuleInfo[2].i4Sensorid = imgsensor_info.sensor_id;
        pCamDeviceObj->i4CurSensorIdx = 2;
        pCamDeviceObj->i4CurSensorId = HI5022Q_SENSOR_ID23618;
    }else{
...
static kal_uint32 return_sensor_id(void)
{
     ...
     return ((read_cmos_sensor(0x0716) << 8) | read_cmos_sensor(0x0717)) +1;////hi5022q id is 5023
}
...
static kal_uint8 read_ark_data_from_eeprom()
{
    //read XGCdata
    kal_uint16 idx = 0;
    int checksum = 0;
    kal_uint16 flag_addr[OTP_FLAG_SIZE] = {0x1520, 0x1A22, 0x2224};
    kal_uint16 checksum_addr[OTP_FLAG_SIZE] = {0x1521, 0x1A23, 0x2225};
    kal_uint16 xgc_startaddr = 0x0DA0;
    kal_uint16 qgc_startaddr = 0x1522;
    kal_uint16 opc_startaddr = 0x1A24;
    for (idx = 0; idx < OTP_FLAG_SIZE; idx++)
    {
       if (read_ark_eeprom_module(flag_addr[idx]) != 1)
       {
           LOG_INF("OPT data read flag err 0x%x", read_ark_eeprom_module(flag_addr[idx]));
           return -1;
       }
   }
    for (idx = 0; idx < hi5022q_OTP_XGC_LEN +1 ; idx++)
    {
        XGCbuffer[idx] = read_ark_eeprom_module(xgc_startaddr + idx);
     //   LOG_INF("read eeprom QGC  addr  : 0x%x, data: 0x%x", xgc_startaddr + idx, XGCbuffer[idx]);
        checksum += XGCbuffer[idx];
    }
    checksum = checksum % 0xFF;
    if (read_ark_eeprom_module(checksum_addr[0])!= checksum)
    {
        LOG_INF("OPT data XGC checksum err data 0x%x addr 0x%x checksum 0x%x", read_ark_eeprom_module(checksum_addr[0]), checksum_addr[0], checksum);
        return -1;
    }
    checksum = 0;
    for (idx = 0; idx < hi5022q_OTP_QGC_LEN + 1; idx++)
    {
        QGCbuffer[idx] = read_ark_eeprom_module(qgc_startaddr + idx);
        checksum += QGCbuffer[idx];
   //     LOG_INF("read eeprom QGC  addr  : 0x%x, data: 0x%x", qgc_startaddr + idx, QGCbuffer[idx]);
    }
    checksum = checksum % 0xFF;
    if (read_ark_eeprom_module(checksum_addr[1])!= checksum)
    {
        LOG_INF("OPT data QGC checksum err data 0x%x addr 0x%x checksum 0x%x", read_ark_eeprom_module(checksum_addr[1]), checksum_addr[1], checksum);
        return -1;
    }
    checksum = 0;
    for (idx = 0; idx < hi5022q_EEPROM_OPC_LEN_WAY1 + 1; idx++)
    {
        OPCbuffer[idx] = read_ark_eeprom_module(opc_startaddr + idx);
  //      LOG_INF("read eeprom QGC  addr  : 0x%x, data: 0x%x", opc_startaddr + idx, OPCbuffer[idx]);
        checksum += OPCbuffer[idx];
    }
    checksum = checksum % 0xFF;
    if (read_ark_eeprom_module(checksum_addr[2])!= checksum)
    {
        LOG_INF("OPT data OPC checksum err data 0x%x addr 0x%x checksum 0x%x", read_ark_eeprom_module(checksum_addr[2]), checksum_addr[2], checksum);
        return -1;
    }
    return 0;
}

/*hs14 code for AL6528ADEU-768 by majunfeng at 2022-10-13 start*/
         ...
         do {
             *sensor_id = return_sensor_id();
             if (*sensor_id == imgsensor_info.sensor_id)
             {
                ...
                read_ark_module_data();
                err = read_ark_data_from_eeprom();
                return ERROR_NONE;
             }
             retry--;
         } while (retry > 0);
...
/*hs14 code for AL6528ADEU-768 by majunfeng at 2022-10-13 end*/
/*************************************************************************
  * FUNCTION
  *    open
  *
     ...
     case SENSOR_FEATURE_GET_SENSOR_PDAF_CAPACITY:
        ...
        case SENSOR_FEATURE_GET_PDAF_DATA:
         ...
         case SENSOR_FEATURE_GET_PDAF_INFO:
             ...
             {
                 case MSDK_SCENARIO_ID_CAMERA_CAPTURE_JPEG:
                     imgsensor_pd_info.i4BlockNumX=512;
                     imgsensor_pd_info.i4BlockNumY=384;
                 ...
                 case MSDK_SCENARIO_ID_VIDEO_PREVIEW:
                     imgsensor_pd_info.i4BlockNumX=512;
                     imgsensor_pd_info.i4BlockNumY=288;
                 memcpy((void *)PDAFinfo, (void*)&imgsensor_pd_info_16_9, sizeof(struct SET_PD_BLOCK_INFO_T));
                 break;
                 case MSDK_SCENARIO_ID_HIGH_SPEED_VIDEO:
                 case MSDK_SCENARIO_ID_SLIM_VIDEO:
...
/*hs14 code for AL6528ADEU-1243 by jianghongyan at 2022-10-28 end*/
