...
#define PFX "ATOM_OV50D40_camera_sensor"
...
#include "atom_ov50d40mipiraw_Sensor.h"
#include "atom_ov50d40_Sensor_setting.h"
...
static struct imgsensor_info_struct imgsensor_info = {
    .sensor_id = ATOM_OV50D40_SENSOR_ID,
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
            *sensor_id = return_sensor_id()+1;
            LOG_INF("[atom_ov50d40] get_imgsensor_id:0x%x  i2c:0x%x\n", *sensor_id, imgsensor.i2c_write_id);
            if (*sensor_id == imgsensor_info.sensor_id) {
                LOG_INF("[atom_ov50d40] i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id, *sensor_id);
                ov50d40_vcm_id = Eeprom_1ByteDataRead(0x0A, 0xA0);
                LOG_INF("[atom_ov50d40] vcm_id = 0x%x\n", ov50d40_vcm_id);
                if(ov50d40_vcm_id == VCM_REG_ATOM) {
                    Eeprom_DataInit(IMGSENSOR_SENSOR_IDX_MAIN, *sensor_id);
                    LOG_INF("[atom_ov50d40] Eeprom_DataInit successful \n");
                } else {
                    LOG_INF("[atom_ov50d40] Not Find vcm_id! \n");
