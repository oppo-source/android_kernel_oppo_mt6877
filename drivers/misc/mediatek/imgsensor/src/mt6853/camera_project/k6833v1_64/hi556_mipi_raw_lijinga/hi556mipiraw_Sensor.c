...
#define PFX "hi556a_camera_sensor"
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = HI556_SENSOR_ID_LIJINGA,
...
static kal_uint16 hi556a_table_write_cmos_sensor(
...
static int module_flag_addr_tbl[MAX_GROUP_NUM] = {0x0410, 0x0C5E, 0x14AC};
static int module_data_addr_tbl[MAX_GROUP_NUM] = {0x0401, 0x0C4F, 0x149D};

static int awb_flag_addr_tbl[MAX_GROUP_NUM] = {0x0431, 0x0C7F, 0x14CD};
static int awb_data_addr_tbl[MAX_GROUP_NUM] = {0x0421, 0x0C6F, 0x14BD};

static int lsc_flag_addr_tbl[MAX_GROUP_NUM] = {0x0B7F, 0x13CD, 0x1C1B};
static int lsc_data_addr_tbl[MAX_GROUP_NUM] = {0x0433, 0x0C81, 0x14CF};

static int SN_flag_addr_tbl[MAX_GROUP_NUM] = {0x0C4D, 0x149B, 0x1CE9};
static int SN_data_addr_tbl[MAX_GROUP_NUM] = {0x0C35, 0x1483, 0x1CD1};
//-bug604664, liuxingyu, add, 2020/12/18, add for hi556 eeprom bring up

#define VALID_OTP_FLAG 0x55
//#define OTP_GROUP_OFFSET 0x76A

unsigned char hi556a_data_lsc[LSC_DATA_SIZE + 2] = {0};/*Add flag and check sum*/
unsigned char hi556a_data_awb[AWB_DATA_SIZE + 2] = {0};/*Add  flag andcheck sum*/
unsigned char hi556a_data_info[MODULE_INFO_SIZE + 2] = {0};/*Add  flag and check sum*/
unsigned char hi556a_data_SN[SN_DATA_SIZE + 2] = {0};
unsigned char hi556a_module_id = 0;
unsigned char hi556a_lsc_valid = 0;
unsigned char hi556a_awb_valid = 0;

static void hi556a_disable_otp_func(void)
{
...
static int read_hi556a_module_info(void)
{
	...
	if (minfo_start_addr != 0) {
		...
		for(i = 0; i < MODULE_INFO_SIZE + 2; i++){  // add flag and checksum
			hi556a_data_info[i]=read_cmos_sensor(OTP_READ_FLAG);
		}
		for(i = 0; i < MODULE_INFO_SIZE; i++){
			check_sum_cal += hi556a_data_info[i];
		}

		check_sum_cal = check_sum_cal % 255;
		hi556a_module_id = (hi556a_data_info[1] << 8) || hi556a_data_info[0];
		day = hi556a_data_info[2];
		month = hi556a_data_info[3];
		year = (hi556a_data_info[5] << 8) || hi556a_data_info[4];
		sensor_id = (hi556a_data_info[7] << 8) || hi556a_data_info[6];
		lens_id = (hi556a_data_info[9] << 8) || hi556a_data_info[8];
		vcm_id = (hi556a_data_info[11] << 8) || hi556a_data_info[10];
		vcm_id2 = (hi556a_data_info[13] << 8) || hi556a_data_info[12];
		mirror_flip = hi556a_data_info[14];
		check_sum = hi556a_data_info[MODULE_INFO_SIZE + 1];

	#ifdef HI556_OTP_DUMP
		dumpEEPROMData1(module_data_addr_tbl[i], MODULE_INFO_SIZE, &hi556a_data_info[0]);
	#endif
...
static int read_hi556a_awb_info(void)
{
	...
	if (awb_start_addr != 0) {
		...
		for(i = 0; i < AWB_DATA_SIZE + 2; i++){ // add flag and checksum
			hi556a_data_awb[i]=read_cmos_sensor(OTP_READ_FLAG);
		}
		for(i = 0; i < AWB_DATA_SIZE; i++){
			check_sum_awb_cal += hi556a_data_awb[i];
		}
		LOG_INF("check_sum_awb_cal =0x%x \n",check_sum_awb_cal);
		r = ((hi556a_data_awb[1]<<8)&0xff00)|(hi556a_data_awb[0]&0xff);
		b = ((hi556a_data_awb[3]<<8)&0xff00)|(hi556a_data_awb[2]&0xff);
		gr = ((hi556a_data_awb[5]<<8)&0xff00)|(hi556a_data_awb[4]&0xff);
		gb = ((hi556a_data_awb[7]<<8)&0xff00)|(hi556a_data_awb[6]&0xff);
		golden_r = ((hi556a_data_awb[9]<<8)&0xff00)|(hi556a_data_awb[8]&0xff);
		golden_b = ((hi556a_data_awb[11]<<8)&0xff00)|(hi556a_data_awb[10]&0xff);
		golden_gr = ((hi556a_data_awb[13]<<8)&0xff00)|(hi556a_data_awb[12]&0xff);
		golden_gb = ((hi556a_data_awb[15]<<8)&0xff00)|(hi556a_data_awb[14]&0xff);
		check_sum_awb = hi556a_data_awb[AWB_DATA_SIZE + 1];
		check_sum_awb_cal = check_sum_awb_cal % 255;

	#ifdef HI556_OTP_DUMP
		dumpEEPROMData1(awb_start_addr, AWB_DATA_SIZE,&hi556a_data_awb[0]);
	#endif
...
static int read_hi556a_lsc_info(void)
{
	...
	if (lsc_start_addr != 0) {
		...
		{
			hi556a_data_lsc[i] = read_cmos_sensor(OTP_READ_FLAG);
		}
		for(i = 0; i < LSC_DATA_SIZE; i++){
			check_sum_lsc_cal += hi556a_data_lsc[i];
		}
		LOG_INF("check_sum_awb_cal =0x%x \n",check_sum_lsc_cal);
		check_sum_lsc = hi556a_data_lsc[LSC_DATA_SIZE + 1];
		check_sum_lsc_cal = check_sum_lsc_cal % 255;

	#ifdef HI556_OTP_DUMP
		dumpEEPROMData1(lsc_start_addr, LSC_DATA_SIZE,&hi556a_data_lsc[0]);
	#endif
...
static int read_hi556a_SN_Code(void)
{
    ...
    if (SN_start_addr != 0) {
        ...
        {
            hi556a_data_SN[i] = read_cmos_sensor(OTP_READ_FLAG);
        }
        for(i = 0; i < SN_DATA_SIZE; i++){
           check_sum_sn_cal += hi556a_data_SN[i];
        }
        LOG_INF("check_sum_sn_cal =0x%x \n",check_sum_sn_cal);
        check_sum_sn = hi556a_data_SN[SN_DATA_SIZE+1];
        check_sum_sn_cal = check_sum_sn_cal % 255;

    #ifdef HI556_OTP_DUMP
        dumpEEPROMData1(SN_start_addr, SN_DATA_SIZE,&hi556a_data_SN[0]);
    #endif
        LOG_INF("=== HI556 SN SN_flag=0x%x ===\n", hi556a_data_SN[SN_DATA_SIZE]);
        if (hi556a_data_SN[SN_DATA_SIZE] == 0x01 && check_sum_sn == check_sum_sn_cal) {
            LOG_INF("check_sum_sn success!\n");
            for (i = 0; i < SN_DATA_SIZE + 2; i++) {
                pCamDeviceObj->camNormdata[1][8+i] = (kal_uint8)hi556a_data_SN[i];
            }
            pCamDeviceObj->pCamModuleInfo[1].i4Sensorid = imgsensor_info.sensor_id;
            pCamDeviceObj->i4CurSensorIdx = 1;
            pCamDeviceObj->i4CurSensorId = HI556_SENSOR_ID_LIJINGA;
            return 1;
        } else {
...
static int hi556a_sensor_otp_info(void)
{
	int ret = 0;
	...
	/* 3. read eeprom data */
	//minfo && awb group
	ret = read_hi556a_module_info();
	if(ret != 1){
		hi556a_module_id = 0;
		LOG_INF("=== hi556a_data_info invalid ===\n");
	}
	ret = read_hi556a_awb_info();
	if(ret != 1){
		hi556a_awb_valid = 0;
		LOG_INF("=== hi556a_data_awb invalid ===\n");
	}else{
		hi556a_awb_valid = 1;
	}
	ret = read_hi556a_lsc_info();
	if(ret != 1){
		hi556a_lsc_valid = 0;
		LOG_INF("=== hi556a_data_lsc invalid ===\n");
	}else{
		hi556a_lsc_valid = 1;
	}
	ret = read_hi556a_SN_Code();
	/* 4. disable otp function */
	hi556a_disable_otp_func();
	if(hi556a_module_id == 0 || hi556a_lsc_valid == 0 || hi556a_awb_valid == 0){
		return 0;
	}else{
		return 1;
	}
...
{
	return (((read_cmos_sensor(0x0F16) << 8) | read_cmos_sensor(0x0F17)) + 0xb);
}
...
#if MULTI_WRITE
kal_uint16 addr_data_pair_init_hi556a[] = {
	0x0a00, 0x0000,
	0x0e00, 0x0102,
	0x0e02, 0x0102,
	0x0e0c, 0x0100,
...
{
#if MULTI_WRITE
	hi556a_table_write_cmos_sensor(
		addr_data_pair_init_hi556a,
		sizeof(addr_data_pair_init_hi556a) /
		sizeof(kal_uint16));
#else
	...
	write_cmos_sensor(0x0f30, 0x8135);
	...
	write_cmos_sensor(0x095a, 0x4920);
...
#if MULTI_WRITE
kal_uint16 addr_data_pair_preview_30fps_hi556a[] = {
	0x0b0a, 0x8252,
	0x0f30, 0x8135,//6925,
	0x0f32, 0x7067,
	0x004a, 0x0100,
...
#if MULTI_WRITE
	hi556a_table_write_cmos_sensor(
		addr_data_pair_preview_30fps_hi556a,
		sizeof(addr_data_pair_preview_30fps_hi556a) /
		sizeof(kal_uint16));
#else
	LOG_INF("%s fps = 300\n", __func__);
	write_cmos_sensor(0x0b0a, 0x8252);
	write_cmos_sensor(0x0f30, 0x8135);//6925);
	write_cmos_sensor(0x0f32, 0x7067);
	write_cmos_sensor(0x004a, 0x0100);
	write_cmos_sensor(0x004c, 0x0000);
...
#if MULTI_WRITE
kal_uint16 addr_data_pair_capture_fps_hi556a[] = {
	0x0b0a, 0x8252,
	0x0f30, 0x8135,//6925,
	0x0f32, 0x7067,
	0x004a, 0x0100,
...
kal_uint16 addr_data_pair_capture_30fps_hi556a[] = {
	0x0b0a, 0x8252,
	0x0f30, 0x8135,//6925,
	0x0f32, 0x7067,
	0x004a, 0x0100,
...
kal_uint16 addr_data_pair_normal_video_30fps_hi556a[] = {
0x0a00, 0x0000,
0x0b0a, 0x8252,
0x0f30, 0x8135,//6925,
0x0f32, 0x7067,
...
static void video_setting(kal_uint16 currefps)
{
	...
	if (currefps == 300) {
		hi556a_table_write_cmos_sensor(
			addr_data_pair_normal_video_30fps_hi556a,
			sizeof(addr_data_pair_normal_video_30fps_hi556a) /
			sizeof(kal_uint16));
	} else {
		hi556a_table_write_cmos_sensor(
			addr_data_pair_capture_fps_hi556a,
			sizeof(addr_data_pair_capture_fps_hi556a) /
			sizeof(kal_uint16));
	}
#else
	if (currefps == 300) {
...
static void capture_setting(kal_uint16 currefps)
{
	...
	if (currefps == 300) {
		hi556a_table_write_cmos_sensor(
			addr_data_pair_capture_30fps_hi556a,
			sizeof(addr_data_pair_capture_30fps_hi556a) /
			sizeof(kal_uint16));
	} else {
		hi556a_table_write_cmos_sensor(
			addr_data_pair_capture_fps_hi556a,
...
#if MULTI_WRITE
kal_uint16 addr_data_pair_hs_video_hi556a[] = {
	0x0b0a, 0x8252,
	0x0f30, 0x8135,
	0x0f32, 0x7267,
	0x004a, 0x0100,
...
#if MULTI_WRITE
	hi556a_table_write_cmos_sensor(
		addr_data_pair_hs_video_hi556a,
		sizeof(addr_data_pair_hs_video_hi556a) /
		sizeof(kal_uint16));
#else
	write_cmos_sensor(0x0b0a, 0x8252);
	write_cmos_sensor(0x0f30, 0x8135);
...
#if MULTI_WRITE
kal_uint16 addr_data_pair_slim_video_hi556a[] = {
	0x0b0a, 0x8252,
	0x0f30, 0x8135,
	0x0f32, 0x7167,
	0x004a, 0x0100,
...
#if MULTI_WRITE
	hi556a_table_write_cmos_sensor(
		addr_data_pair_slim_video_hi556a,
		sizeof(addr_data_pair_slim_video_hi556a) /
		sizeof(kal_uint16));
#else
	write_cmos_sensor(0x0b0a, 0x8252);
	write_cmos_sensor(0x0f30, 0x8135);
