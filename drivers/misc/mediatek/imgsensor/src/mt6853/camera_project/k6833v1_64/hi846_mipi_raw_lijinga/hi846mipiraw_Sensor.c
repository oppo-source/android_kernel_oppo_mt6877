...
#define PFX "hi846a_camera_sensor"
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = HI846_SENSOR_ID_LIJINGA,
	...
	.pre = {
		...
		.max_framerate = 300,
		.mipi_pixel_rate = 272000000, //(360M*4/10)
	},
	.cap = {
		.pclk = 288000000,
		.linelength = 3800,
		...
		.mipi_data_lp2hs_settle_dc = 85,
		.max_framerate = 300,
		.mipi_pixel_rate = 272000000, //(720M*4/10)
	},
	.cap1 = {
		.pclk = 288000000,
		.linelength = 3800,
	...
	.normal_video = {
		...
		.framelength = 2512,			//record different mode's framelength
	...
	.hs_video = {
		...
		.max_framerate = 900,
		.mipi_pixel_rate = 136000000, //(180M*4/10)
	},
	.slim_video = {
		.pclk = 288000000,
		.linelength = 3800,
	...
	.custom2 = {
		...
		/*	 following for GetDefaultFramerateByScenario()	*/
		.max_framerate = 300,
		.mipi_pixel_rate = 136000000, //(360M*4/10)
	},
	.custom3 = {
		.pclk = 288000000,
		.linelength = 3800,
...
static struct SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[8] =
{
 ...
 { 3264, 2448,   0, 308,   3264, 1836,   3264, 1836,   0, 0, 3264, 1836,   0, 0, 3264, 1836}, // video
...
static kal_uint16 hi846a_table_write_cmos_sensor(
...
//+huangmiao.wt,ADD,2022/6/22,hi846a eeprom bring up
//#define HI846_OTP_DUMP

#ifdef HI846_OTP_DUMP
...
static int module_flag_addr_tbl[MAX_GROUP_NUM] = {0x0210, 0x0221, 0x0232};
static int module_data_addr_tbl[MAX_GROUP_NUM] = {0x0201, 0x0212, 0x0223};

static int awb_flag_addr_tbl[MAX_GROUP_NUM] = {0x0244, 0x0256, 0x0268};
static int awb_data_addr_tbl[MAX_GROUP_NUM] = {0x0234, 0x0246, 0x0258};

static int lsc_flag_addr_tbl[MAX_GROUP_NUM] = {0x09B6, 0x1104, 0x1852};
static int lsc_data_addr_tbl[MAX_GROUP_NUM] = {0x026A, 0x09B8, 0x1106};

static int sn_flag_addr_tbl[MAX_GROUP_NUM] = {0x186C, 0x1886, 0x18A0};
static int sn_data_addr_tbl[MAX_GROUP_NUM] = {0x1854, 0x186E, 0x1888};

#define VALID_OTP_FLAG 0x01
//#define OTP_GROUP_OFFSET 0x76A

unsigned char hi846a_data_lsc[LSC_DATA_SIZE + 2] = {0};/*Add flag and check sum*/
unsigned char hi846a_data_awb[AWB_DATA_SIZE + 2] = {0};/*Add flag and check sum*/
unsigned char hi846a_data_info[MODULE_INFO_SIZE + 2] = {0};/*Add flag and check sum*/
unsigned char hi846a_data_sn[SN_DATA_SIZE + 2] = {0};/*Add flag and check sum*/

unsigned char hi846a_module_id = 0;
unsigned char hi846a_lsc_valid = 0;
unsigned char hi846a_awb_valid = 0;
unsigned char hi846a_sn_valid = 0;

static void hi846a_disable_otp_func(void)
{
...
static int read_hi846a_module_info(void)
{
	...
	if(minfo_start_addr != 0){
		...
		for(i = 0; i < MODULE_INFO_SIZE + 2; i++){  // add flag and checksum
			hi846a_data_info[i]=read_cmos_sensor(OTP_READ_FLAG);
		}
		for(i = 0; i < MODULE_INFO_SIZE; i++){
			check_sum_cal += hi846a_data_info[i];
		}

		check_sum_cal = check_sum_cal % 255;
		hi846a_module_id = (hi846a_data_info[1] << 8) || hi846a_data_info[0];
		day = hi846a_data_info[2];
		month = hi846a_data_info[3];
		year = (hi846a_data_info[5] << 8) || hi846a_data_info[4];
		sensor_id = (hi846a_data_info[7] << 8) || hi846a_data_info[6];
		lens_id = (hi846a_data_info[9] << 8) || hi846a_data_info[8];
		vcm_id = (hi846a_data_info[11] << 8) || hi846a_data_info[10];
		vcm_id2 = (hi846a_data_info[13] << 8) || hi846a_data_info[12];
		mirror_flip = hi846a_data_info[14];
		check_sum = hi846a_data_info[MODULE_INFO_SIZE + 1];

	#ifdef HI846_OTP_DUMP
		dumpEEPROMData1(minfo_start_addr, MODULE_INFO_SIZE, &hi846a_data_info[0]);
	#endif
		LOG_INF("=== HI846 INFO module_id=0x%x ===\n", hi846a_module_id);
		LOG_INF("=== HI846 INFO date is %d-%d-%d ===\n",year,month,day);
		LOG_INF("=== HI846 INFO sensor_id=0x%x, lens_id=0x%x,vcm_id=0x%x,vcm_id2=0x%x ===\n",sensor_id, lens_id, vcm_id, vcm_id2);
		LOG_INF("=== HI846 INFO mirror_flip=0x%x ===\n", mirror_flip);
...
static int read_hi846a_awb_info(void)
{
	...
	if (awb_start_addr != 0) {
		...
		for(i = 0; i < AWB_DATA_SIZE + 2; i++){ // add flag and checksum
			hi846a_data_awb[i]=read_cmos_sensor(OTP_READ_FLAG);
		}
		for(i = 0; i < AWB_DATA_SIZE; i++){
			check_sum_awb_cal += hi846a_data_awb[i];
		}
		LOG_INF("check_sum_awb_cal =0x%x \n",check_sum_awb_cal);
		r = ((hi846a_data_awb[1]<<8)&0xff00)|(hi846a_data_awb[0]&0xff);
		gr = ((hi846a_data_awb[3]<<8)&0xff00)|(hi846a_data_awb[2]&0xff);
		gb = ((hi846a_data_awb[5]<<8)&0xff00)|(hi846a_data_awb[4]&0xff);
		b = ((hi846a_data_awb[7]<<8)&0xff00)|(hi846a_data_awb[6]&0xff);
		golden_r = ((hi846a_data_awb[9]<<8)&0xff00)|(hi846a_data_awb[8]&0xff);
		golden_gr = ((hi846a_data_awb[11]<<8)&0xff00)|(hi846a_data_awb[10]&0xff);
		golden_gb = ((hi846a_data_awb[13]<<8)&0xff00)|(hi846a_data_awb[12]&0xff);
		golden_b = ((hi846a_data_awb[15]<<8)&0xff00)|(hi846a_data_awb[14]&0xff);
		check_sum_awb = hi846a_data_awb[AWB_DATA_SIZE + 1];
		check_sum_awb_cal = check_sum_awb_cal % 255;

		#ifdef HI846_OTP_DUMP
		dumpEEPROMData1(awb_start_addr, AWB_DATA_SIZE,&hi846a_data_awb[0]);
		#endif
		LOG_INF("=== hi846a AWB r=0x%x, b=0x%x, gr=%x, gb=0x%x ===\n", r, b,gb, gr);
		LOG_INF("=== hi846a AWB gr=0x%x,gb=0x%x,gGr=%x, gGb=0x%x ===\n", golden_r, golden_b, golden_gr, golden_gb);
		LOG_INF("=== hi846a AWB check_sum_awb=0x%x,check_sum_awb_cal=0x%x ===\n",check_sum_awb,check_sum_awb_cal);
		if(check_sum_awb == check_sum_awb_cal){
			LOG_INF("check_sum_awb success!\n");
			return 1;
		}else{
			return 0;
			LOG_INF("check_sum_awb fail!\n");
		}
	} else {
		return 0;
	}
}
static int read_hi846a_lsc_info(void)
{
	...
	if (lsc_start_addr != 0) {
		...
		{
		  hi846a_data_lsc[i] = read_cmos_sensor(OTP_READ_FLAG);
		}
		for(i = 0; i < LSC_DATA_SIZE; i++){
		  check_sum_lsc_cal += hi846a_data_lsc[i];
		}
		LOG_INF("check_sum_lsc_cal =0x%x \n",check_sum_lsc_cal);
		check_sum_lsc = hi846a_data_lsc[LSC_DATA_SIZE + 1];
		check_sum_lsc_cal = check_sum_lsc_cal % 255;

		#ifdef HI846_OTP_DUMP
		dumpEEPROMData1(lsc_start_addr, LSC_DATA_SIZE,&hi846a_data_lsc[0]);
		#endif
		LOG_INF("=== hi846a LSC check_sum_lsc=0x%x, check_sum_lsc_cal=0x%x ===\n", check_sum_lsc, check_sum_lsc_cal);
		if(check_sum_lsc == check_sum_lsc_cal){
			LOG_INF("check_sum_lsc success!\n");
			return 1;
		}else{
			return 0;
			LOG_INF("check_sum_lsc fail!\n");
		}
	} else {
		return 0;
	}
}
static int read_hi846a_sn_info(void)
{
	...
	for(i = 0; i < MAX_GROUP_NUM; i++){
		...
		otp_grp_flag = read_cmos_sensor(OTP_READ_FLAG);
		if (otp_grp_flag == 0x01) {
			LOG_INF("hi846_sn: group %d selected\n", i);
	...
	if(sn_start_addr != 0){
		...
		{
		  hi846a_data_sn[i] = read_cmos_sensor(OTP_READ_FLAG);
		}
		for(i = 0; i < SN_DATA_SIZE; i++){
		  check_sum_sn_cal += hi846a_data_sn[i];
		}
		LOG_INF("check_sum_sn_cal =0x%x \n",check_sum_sn_cal);
		check_sum_sn = hi846a_data_sn[SN_DATA_SIZE+1];
		check_sum_sn_cal = check_sum_sn_cal % 255;

		#ifdef HI846_OTP_DUMP
		dumpEEPROMData1(sn_start_addr, SN_DATA_SIZE,&hi846a_data_sn[0]);
		#endif
		LOG_INF("=== hi846a SN check_sum_sn=0x%x, check_sum_sn_cal=0x%x ===\n", check_sum_sn, check_sum_sn_cal);
		if(check_sum_sn == check_sum_sn_cal){
			LOG_INF("check_sum_sn success!\n");
			for (i = 0; i < SN_DATA_SIZE + 2; i++) { //add flag and checksum
				pCamDeviceObj->camNormdata[1][8+i] = (kal_uint8)hi846a_data_sn[i];
			}
			pCamDeviceObj->pCamModuleInfo[1].i4Sensorid = imgsensor_info.sensor_id;
			pCamDeviceObj->i4CurSensorIdx = 1;
			pCamDeviceObj->i4CurSensorId = HI846_SENSOR_ID_LIJINGA;
			return 1;
		}else{
			return 0;
			LOG_INF("check_sum_sn fail!\n");
		}
	} else {
		return 0;
	}
...
static int hi846a_sensor_otp_info(void)
{
	int ret = 0;

	LOG_INF("come to hi846_sensor_otp_info %s:%d E!\n", __func__, __LINE__);

	/* 1. sensor init */
	write_cmos_sensor(0x0A00, 0x0000); //tg_pmem_sckpw/sdly
	write_cmos_sensor(0x2000, 0x0000); //tg_pmem_sckpw/sdly
	write_cmos_sensor(0x2002, 0x00FF); //tg_pmem_sckpw/sdly
	write_cmos_sensor(0x2004, 0x0000); //tg_pmem_rom_dly
	write_cmos_sensor(0x2008, 0x3FFF); // firmware start address-ROM
	write_cmos_sensor(0x23FE, 0xC056); // BGR enable
	write_cmos_sensor(0x0A00, 0x0000); // STRB(OTP Busy) output enable
	write_cmos_sensor(0x0E04, 0x0012); // STRB(OTP Busy) output drivability
	write_cmos_sensor(0x0F08, 0x2F04); // Analog PLL setting
	write_cmos_sensor(0x0F30, 0x001F); // Analog CLKGEN setting
	write_cmos_sensor(0x0F36, 0x001F); // Analog CLKGEN setting
	write_cmos_sensor(0x0F04, 0x3A00); // PLL enable
	write_cmos_sensor(0x0F32, 0x025A); // mipi disable
	write_cmos_sensor(0x0F38, 0x0256); // TG PMEM CEN anable
	write_cmos_sensor(0x0F2A, 0x4124); // TG MCU enable
	write_cmos_sensor(0x006A, 0x0100); // ROM OTP Continuous W/R mode enable
	write_cmos_sensor(0x004C, 0x0100); // Stream ON
	...
	/* 3. read eeprom data */
	//minfo && awb &&lsc group
	ret = read_hi846a_module_info();
	if(ret != 1){
		hi846a_module_id = 0;
		LOG_INF("=== hi846a_data_info invalid ===\n");
	}
	ret = read_hi846a_awb_info();
	if(ret != 1){
		hi846a_awb_valid = 0;
		LOG_INF("=== hi846a_data_awb invalid ===\n");
	}else{
		hi846a_awb_valid = 1;
	}
	ret = read_hi846a_lsc_info();
	if(ret != 1){
		hi846a_lsc_valid = 0;
		LOG_INF("=== hi846a_data_lsc invalid ===\n");
	}else{
		hi846a_lsc_valid = 1;
	}
	ret = read_hi846a_sn_info();
	if(ret != 1){
		hi846a_sn_valid = 0;
		LOG_INF("=== hi846a_data_sn invalid ===\n");
	}else{
		hi846a_sn_valid = 1;
	}

	/* 4. disable otp function */
	hi846a_disable_otp_func();
	if(hi846a_module_id == 0 || hi846a_lsc_valid == 0 || hi846a_awb_valid == 0 || hi846a_sn_valid == 0){
		return 0;
	}else{
		return 1;
	}
...
#if MULTI_WRITE
kal_uint16 addr_data_pair_init_hi846a[] = {
	0x0066, 0x0101,
	0x2000, 0x98E8,
	0x2002, 0x00FF,
	0x2004, 0x0006,
...
{
#if MULTI_WRITE
	hi846a_table_write_cmos_sensor(
		addr_data_pair_init_hi846a,
		sizeof(addr_data_pair_init_hi846a) /
		sizeof(kal_uint16));
#else
...
#if MULTI_WRITE
kal_uint16 addr_data_pair_preview_hi846a[] = {
	0x002E, 0x1111,
	0x0032, 0x1111,
	0x0026, 0x0040,
	0x002C, 0x09CF,
...
#if MULTI_WRITE
	hi846a_table_write_cmos_sensor(
		addr_data_pair_preview_hi846a,
		sizeof(addr_data_pair_preview_hi846a) /
		sizeof(kal_uint16));
#else
	LOG_INF("E preview\n");
	////////////////////////////////////////////////////////////
...
#if MULTI_WRITE
kal_uint16 addr_data_pair_capture_fps_hi846a[] = {
	0x002E, 0x1111,
	0x0032, 0x1111,
	0x0026, 0x0040,
	0x002C, 0x09CF,
...
kal_uint16 addr_data_pair_capture_30fps_hi846a[] = {
	0x002E, 0x1111,
	0x0032, 0x1111,
	0x0026, 0x0040,
	0x002C, 0x09CF,
...
static void capture_setting(kal_uint16 currefps)
{
	...
	if (currefps == 300) {
	hi846a_table_write_cmos_sensor(
		addr_data_pair_capture_30fps_hi846a,
		sizeof(addr_data_pair_capture_30fps_hi846a) /
		sizeof(kal_uint16));
	} else {
	hi846a_table_write_cmos_sensor(
		addr_data_pair_capture_fps_hi846a,
		sizeof(addr_data_pair_capture_fps_hi846a) /
		sizeof(kal_uint16));
	}
#else
	if (currefps == 300) {
...
#if MULTI_WRITE
kal_uint16 addr_data_pair_video_hi846a[] = {
	0x002E, 0x1111,
	0x0032, 0x1111,
	0x0026, 0x0172,
	0x002C, 0x089D,
...
#if MULTI_WRITE
	hi846a_table_write_cmos_sensor(
		addr_data_pair_video_hi846a,
		sizeof(addr_data_pair_video_hi846a) /
		sizeof(kal_uint16));
#else
	//Sensor Information////////////////////////////
	//Sensor	  : Hi-846
...
#if MULTI_WRITE
kal_uint16 addr_data_pair_hs_video_hi846a[] = {
	0x002E, 0x3311,
	0x0032, 0x3311,
	0x0026, 0x0238,
	0x002C, 0x07D7,
...
#if MULTI_WRITE
	hi846a_table_write_cmos_sensor(
		addr_data_pair_hs_video_hi846a,
		sizeof(addr_data_pair_hs_video_hi846a) /
		sizeof(kal_uint16));
#else
...
#if MULTI_WRITE
kal_uint16 addr_data_pair_slim_video_hi846a[] = {
	0x002E, 0x1111,
	0x0032, 0x1111,
	0x0026, 0x0040,
	0x002C, 0x09CF,
...
#if MULTI_WRITE
	hi846a_table_write_cmos_sensor(
		addr_data_pair_slim_video_hi846a,
		sizeof(addr_data_pair_slim_video_hi846a) /
		sizeof(kal_uint16));
#else
	write_cmos_sensor(0x002E, 0x1111);
	write_cmos_sensor(0x0032, 0x1111);
...
#if MULTI_WRITE
kal_uint16 addr_data_pair_custom1_hi846a[] = {
	0x002E, 0x1111,
	0x0032, 0x1111,
	0x0026, 0x0040,
	0x002C, 0x09CF,
...
#if MULTI_WRITE
	hi846a_table_write_cmos_sensor(
		addr_data_pair_custom1_hi846a,
		sizeof(addr_data_pair_custom1_hi846a) /
		sizeof(kal_uint16));
#else
	LOG_INF("E custom1\n");
	write_cmos_sensor(0x002E, 0x1111);
...
#if MULTI_WRITE
kal_uint16 addr_data_pair_custom2_hi846a[] = {
	0x002E, 0x3311,
	0x0032, 0x3311,
	0x0026, 0x0040,
	0x002C, 0x09CF,
...
#if MULTI_WRITE
	hi846a_table_write_cmos_sensor(
		addr_data_pair_custom2_hi846a,
		sizeof(addr_data_pair_custom2_hi846a) /
		sizeof(kal_uint16));
#else
	LOG_INF("E custom2\n");
	write_cmos_sensor(0x002E, 0x3311);
...
#if MULTI_WRITE
kal_uint16 addr_data_pair_custom3_hi846a[] = {
	0x002E, 0x3311,
	0x0032, 0x3311,
	0x0026, 0x0040,
	0x002C, 0x09CF,
...
#if MULTI_WRITE
	hi846a_table_write_cmos_sensor(
		addr_data_pair_custom3_hi846a,
		sizeof(addr_data_pair_custom3_hi846a) /
		sizeof(kal_uint16));
#else
	LOG_INF("E custom3\n");
	//Sensor Information////////////////////////////
...
static void read_cmos_eeprom_table(kal_uint16 addr, kal_uint8 *table, kal_uint32 size)
{
	...
	iReadRegI2C(pusendcmd, 2, (u8 *)table, size, 0x40);
}
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		spin_unlock(&imgsensor_drv_lock);
		do {
			*sensor_id = return_sensor_id() + 10;
			if (*sensor_id == imgsensor_info.sensor_id) {
			LOG_INF("i2c write id : 0x%x, sensor id: 0x%x\n",imgsensor.i2c_write_id, *sensor_id);
				//+huangmiao.wt,ADD,2022/6/22,hi846a eeprom bring up
				rc = hi846a_sensor_otp_info();
				if(rc == 0){
