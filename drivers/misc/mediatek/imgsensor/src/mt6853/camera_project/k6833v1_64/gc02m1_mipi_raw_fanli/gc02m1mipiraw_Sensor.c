...
#define DEVICE_VERSION_FANLI_GC02M1    "gc02m1"
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = GC02M1_SENSOR_ID_FANLI,
...
static BYTE gc02m1_fanli_common_data[CAMERA_EEPPROM_COMDATA_LENGTH] = { 0 };

#if 0
static kal_uint16 read_gc02m1_fanli_eeprom_module(kal_uint32 addr)
{
	kal_uint16 get_byte = 0;
...
static void read_gc02m1_fanli_module_data(UINT32 sensor_id)
{
// gc02m1_fanli_common_data
// length   : 64
...
// QR       : [8 ~ 26]
	...
	memset(gc02m1_fanli_common_data, 0,sizeof(gc02m1_fanli_common_data));
	// QR
	for(idx = 0; idx < sn_length; idx++)
	{
		gc02m1_fanli_common_data[8 + idx] = read_gc02m1_fanli_eeprom_module(sn_starAddr + idx);
	}
	//module id
	gc02m1_fanli_common_data[0] = read_gc02m1_fanli_eeprom_module(moduleAddr);
	gc02m1_fanli_common_data[1] = read_gc02m1_fanli_eeprom_module(moduleAddr + 1);
	//sensor id
	gc02m1_fanli_common_data[2] = read_gc02m1_fanli_eeprom_module(sensorAddr);
	gc02m1_fanli_common_data[3] = read_gc02m1_fanli_eeprom_module(sensorAddr + 1);
	//lens id
	gc02m1_fanli_common_data[4] = read_gc02m1_fanli_eeprom_module(lensAddr);
	gc02m1_fanli_common_data[5] = read_gc02m1_fanli_eeprom_module(lensAddr + 1);
	//vcm id
	gc02m1_fanli_common_data[6] = read_gc02m1_fanli_eeprom_module(vcmAddr);
	gc02m1_fanli_common_data[7] = read_gc02m1_fanli_eeprom_module(vcmAddr + 1);

	for (idx = 0; idx < CAMERA_EEPPROM_COMDATA_LENGTH; idx = idx + 4)
	{
		pr_debug("cam data: %02x %02x %02x %02x\n",
		       gc02m1_fanli_common_data[idx],
		       gc02m1_fanli_common_data[idx + 1],
		       gc02m1_fanli_common_data[idx + 2],
		       gc02m1_fanli_common_data[idx + 3]);
	}
}
#endif
			...
			*sensor_id = (return_sensor_id() + 3);
			if (*sensor_id == imgsensor_info.sensor_id) {
				if (deviceInfo_register_value == 0x00) {
					Eeprom_DataInit(3, GC02M1_SENSOR_ID_FANLI);
					deviceInfo_register_value = 0x01;
				}
				printk("gc02m1 i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id, *sensor_id);
				//read_gc02m1_fanli_module_data(*sensor_id);
				return ERROR_NONE;
			}
	...
	UINT8 *feature_para, UINT32 *feature_para_len)
{
	...
	case SENSOR_FEATURE_GET_EEPROM_COMDATA:
		memcpy(feature_return_para_32, gc02m1_fanli_common_data,
