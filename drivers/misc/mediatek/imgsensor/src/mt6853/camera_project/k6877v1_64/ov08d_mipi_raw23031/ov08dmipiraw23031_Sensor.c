...
#include "ov08dmipiraw23031_Sensor.h"
#define PFX "ov08d_mipiraw_sensor"
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = OV08D_SENSOR_ID23031,
			...
			*sensor_id = return_sensor_id();
			if (*sensor_id == OV08D_SENSOR_ID) {
				*sensor_id = imgsensor_info.sensor_id;
				pr_info("Ov08d Read sensor id Success, write id:0x%x, id: 0x%x\n", imgsensor.i2c_write_id, *sensor_id);
				if (deviceInfo_register_value == 0x00) {
					Eeprom_DataInit(1, OV08D_SENSOR_ID23031);
			...
			pr_info("Ov08d Read sensor id fail, write id:0x%x, id: 0x%x\n", imgsensor.i2c_write_id, *sensor_id);
			retry--;
		} while (retry > 0);
		i++;
		retry = 2;
