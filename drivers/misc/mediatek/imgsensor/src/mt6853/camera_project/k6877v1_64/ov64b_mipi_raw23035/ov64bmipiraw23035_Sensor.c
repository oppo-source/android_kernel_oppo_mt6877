...
#include "ov64bmipiraw23035_Sensor.h"
#include "ov64b_Sensor23035_setting.h"
...
#define PFX "Avatar_OV64B_camera_sensor"
...
static struct imgsensor_info_struct imgsensor_info = {
    .sensor_id = OV64B_SENSOR_ID23035,
...
static struct SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[15] = {
    ...
    {9280, 6944, 2304, 1728, 4672, 3488, 4672, 3488, 24, 10, 4624, 3468, 0, 0, 4624, 3468},  /*custom6*/
...
static struct SET_PD_BLOCK_INFO_T imgsensor_pd_info_fullsize = {
     ...
     .i4SubBlkW = 16,
     .i4SubBlkH = 4,
     .i4PosL = {{13, 16}, {13, 19}, {21, 9}, {21, 11},
                },
     .i4PosR = {{12, 16}, {12, 19}, {20, 9}, {20, 11},
                 },
     .iMirrorFlip = 0,
     .i4BlockNumX = 288,
     .i4BlockNumY = 216,
...
{
    char pusendcmd[2] = {(char)(addr >> 8), (char)(addr & 0xFF) };
    iReadRegI2CTiming(pusendcmd, 2, (u8 *)table, size, EEPROM_I2C_ADDR, imgsensor_info.i2c_speed);
}
            ...
            *sensor_id = return_sensor_id();
            if (*sensor_id == OV64B_SENSOR_ID) {
                *sensor_id = imgsensor_info.sensor_id;
                pr_info("Avatar Ov64b Read sensor id Success, write id:0x%x, id: 0x%x\n", imgsensor.i2c_write_id, *sensor_id);
                read_EepromQSC();
                if (deviceInfo_register_value == 0x00) {
                    Eeprom_DataInit(0, OV64B_SENSOR_ID23035);
                    deviceInfo_register_value = 0x01;
                }
                if (otp_data[0] == 0) {
                    read_otp_info(otp_data);
                } else {
                    pr_info("Avatar otp data has already read");
                }
                return ERROR_NONE;
            }
            pr_info("Avatar Ov64b Read sensor id fail, write id:0x%x, id: 0x%x\n", imgsensor.i2c_write_id, *sensor_id);
            retry--;
        } while (retry > 0);
        i++;
        retry = 2;
...
//    int k = 0;
    ...
    if (scenario_id == MSDK_SCENARIO_ID_CAMERA_PREVIEW) {
        if (imgsensor.frame_length && imgsensor_info.custom5.linelength) {
            divisor = imgsensor_info.custom5.pclk / imgsensor.frame_length / imgsensor_info.custom5.linelength;
        }
    } else if (scenario_id == MSDK_SCENARIO_ID_CUSTOM5) {
        if (imgsensor.frame_length && imgsensor_info.pre.linelength) {
            divisor = imgsensor_info.pre.pclk / imgsensor.frame_length / imgsensor_info.pre.linelength;
        }
    }
    if (divisor == 0) {
        LOG_INF("divisor cant be 0");
    } else {
        delay_time = 1000 / divisor;
    }
    mdelay(delay_time);
    LOG_INF("seamless_switch delay: %u", delay_time);
    pr_err("exit\n");
    return ERROR_NONE;
}
...
/*    case SENSOR_FEATURE_CHECK_MODULE_ID:
    ...
    case SENSOR_FEATURE_GET_PDAF_INFO:
        ...
        case MSDK_SCENARIO_ID_CUSTOM5:
        case MSDK_SCENARIO_ID_CUSTOM6:
            memcpy((void *)PDAFinfo, (void *)&imgsensor_pd_info_fullsize,
                sizeof(struct SET_PD_BLOCK_INFO_T));
            break;
        default:
            memcpy((void *)PDAFinfo, (void *)&imgsensor_pd_info,
