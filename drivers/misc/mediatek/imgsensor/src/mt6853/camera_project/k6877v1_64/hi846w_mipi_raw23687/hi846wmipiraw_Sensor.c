...
#include "hi846wmipiraw_Sensor.h"
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = HI846W_SENSOR_ID23687,
	...
	.pre = {
		...
		.max_framerate = 300,
		.mipi_pixel_rate = 275200000,	//(688*4/10) 
	},
	.cap = {
		.pclk = 288000000,
		.linelength = 3800,
	...
	.hs_video = {
		...
		.max_framerate = 900,
		.mipi_pixel_rate = 137600000, //(344M*4/10)
	},
	.slim_video = {
		.pclk = 288000000,
		.linelength = 3800,
	...
	.custom3 = {
		...
		/*	 following for GetDefaultFramerateByScenario()	*/
		.max_framerate = 150,
		.mipi_pixel_rate = 137600000, //(344*4/10)
	},
	.margin = 6,
	.min_shutter = 6,
	.min_gain = 64, /*1x gain*/
...
static struct SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[8] =
{
 ...
 { 3264, 2448,   0, 306,   3264, 1836,   3264, 1836, 408, 0, 2448, 1836,   0,  0, 2448, 1836}, // custom1 Bining 2448x1836
 { 3264, 2448,   0, 306,   3264, 1836,   3264, 1836, 408, 0, 2448, 1836, 612,459, 1224,  918}, // custom2 Bining 1224x918
...
static int module_flag_addr_tbl[MAX_GROUP_NUM] = {0x0211, 0x09A2, 0x1133};
static int module_data_addr_tbl[MAX_GROUP_NUM] = {0x0201, 0x0992, 0x1123};

static int awb_flag_addr_tbl[MAX_GROUP_NUM] = {0x0223, 0x09B4, 0x1145};
static int awb_data_addr_tbl[MAX_GROUP_NUM] = {0x0213, 0x09A4, 0x1135};

static int lsc_flag_addr_tbl[MAX_GROUP_NUM] = {0x0990, 0x1121, 0x18B2};
static int lsc_data_addr_tbl[MAX_GROUP_NUM] = {0x0244, 0x09D5, 0x09D5};

static int sn_flag_addr_tbl[MAX_GROUP_NUM] = {0x0242, 0x09D3, 0x1164};
static int sn_data_addr_tbl[MAX_GROUP_NUM] = {0x022B, 0x09BC, 0x114D};

#define VALID_OTP_FLAG 0x01
//#define OTP_GROUP_OFFSET 0x76A

unsigned char hi846_data_lsc[LSC_DATA_SIZE + 2] = {0};/*Add flag and check sum*/
unsigned char hi846_data_awb[AWB_DATA_SIZE + 2] = {0};/*Add flag and check sum*/
unsigned char hi846_data_info[MODULE_INFO_SIZE + 3] = {0};/*Add flag and check sum and HVS TEST*/
unsigned char hi846_data_sn[SN_DATA_SIZE + 2] = {0};/*Add flag and check sum*/

unsigned char hi846_module_id = 0;
unsigned char hi846_lsc_valid = 0;
unsigned char hi846_awb_valid = 0;
unsigned char hi846_sn_valid = 0;
...
static int read_hi846_module_info(void)
{
	...
	if(minfo_start_addr != 0){
		...
		write_cmos_sensor_8(OTP_RW_FLAG,0x01);
		for(i = 0; i < MODULE_INFO_SIZE + 3; i++){  // add flag and checksum and HVS
			hi846_data_info[i]=read_cmos_sensor(OTP_READ_FLAG);
		}
		...
		hi846_module_id = (hi846_data_info[1] << 8) | hi846_data_info[0];
		day = hi846_data_info[2];
		month = hi846_data_info[3];
		year = (hi846_data_info[5] << 8) | hi846_data_info[4];
		sensor_id = (hi846_data_info[7] << 8) | hi846_data_info[6];
		lens_id = (hi846_data_info[9] << 8) | hi846_data_info[8];
		vcm_id = (hi846_data_info[11] << 8) | hi846_data_info[10];
		vcm_id2 = (hi846_data_info[13] << 8) | hi846_data_info[12];
		mirror_flip = hi846_data_info[14];
		check_sum = hi846_data_info[MODULE_INFO_SIZE + 2];

	#ifdef HI846_OTP_DUMP
		dumpEEPROMData1(minfo_start_addr, MODULE_INFO_SIZE, &hi846_data_info[0]);
	#endif
...
static int read_hi846_awb_info(void)
{
	...
	if (awb_start_addr != 0) {
		...
		for(i = 0; i < AWB_DATA_SIZE + 2; i++){ // add flag and checksum
			hi846_data_awb[i]=read_cmos_sensor(OTP_READ_FLAG);
		}
		...
		gr = ((hi846_data_awb[3]<<8)&0xff00)|(hi846_data_awb[2]&0xff);
		gb = ((hi846_data_awb[5]<<8)&0xff00)|(hi846_data_awb[4]&0xff);
		b = ((hi846_data_awb[7]<<8)&0xff00)|(hi846_data_awb[6]&0xff);
		golden_r = ((hi846_data_awb[9]<<8)&0xff00)|(hi846_data_awb[8]&0xff);
		golden_gr = ((hi846_data_awb[11]<<8)&0xff00)|(hi846_data_awb[10]&0xff);
		golden_gb = ((hi846_data_awb[13]<<8)&0xff00)|(hi846_data_awb[12]&0xff);
		golden_b = ((hi846_data_awb[15]<<8)&0xff00)|(hi846_data_awb[14]&0xff);
		check_sum_awb = hi846_data_awb[AWB_DATA_SIZE + 1];
		check_sum_awb_cal = check_sum_awb_cal % 255;

		#ifdef HI846_OTP_DUMP
		dumpEEPROMData1(awb_start_addr, AWB_DATA_SIZE,&hi846_data_awb[0]);
		#endif
...
static int read_hi846_lsc_info(void)
{
	...
	if (lsc_start_addr != 0) {
		...
		{
		  hi846_data_lsc[i] = read_cmos_sensor(OTP_READ_FLAG);
		}
		for(i = 0; i < LSC_DATA_SIZE; i++){
		  check_sum_lsc_cal += hi846_data_lsc[i];
		}
		LOG_INF("check_sum_lsc_cal =0x%x \n",check_sum_lsc_cal);
		check_sum_lsc = hi846_data_lsc[LSC_DATA_SIZE + 1];
		check_sum_lsc_cal = check_sum_lsc_cal % 255;

		#ifdef HI846_OTP_DUMP
		dumpEEPROMData1(lsc_start_addr, LSC_DATA_SIZE,&hi846_data_lsc[0]);
		#endif
...
static int read_hi846_sn_info(void)
{
	...
	if(sn_start_addr != 0){
		...
		{
		  hi846_data_sn[i] = read_cmos_sensor(OTP_READ_FLAG);
		}
		for(i = 0; i < SN_DATA_SIZE; i++){
		  check_sum_sn_cal += hi846_data_sn[i];
		}
		LOG_INF("check_sum_sn_cal =0x%x \n",check_sum_sn_cal);
		check_sum_sn = hi846_data_sn[SN_DATA_SIZE+1];
		check_sum_sn_cal = check_sum_sn_cal % 255;

		#ifdef HI846_OTP_DUMP
		dumpEEPROMData1(sn_start_addr, SN_DATA_SIZE,&hi846_data_sn[0]);
		#endif
		LOG_INF("=== hi846 SN check_sum_sn=0x%x, check_sum_sn_cal=0x%x ===\n", check_sum_sn, check_sum_sn_cal);
		if(check_sum_sn == check_sum_sn_cal){
			LOG_INF("check_sum_sn success!\n");
			for (i = 0; i < SN_DATA_SIZE; i++) {
				pCamDeviceObj->camNormdata[1][8+i] = (kal_uint8)hi846_data_sn[i];
			}
			pCamDeviceObj->pCamModuleInfo[1].i4Sensorid = imgsensor_info.sensor_id;
			pCamDeviceObj->i4CurSensorIdx = 1;
			pCamDeviceObj->i4CurSensorId = HI846W_SENSOR_ID23687;
			return 1;
		}else{
			return 0;
...
static int hi846_sensor_otp_info(void)
{
	...
	if(ret != 1){
		hi846_module_id = 0;
		LOG_INF("=== hi846_data_info invalid ===\n");
	}
	ret = read_hi846_awb_info();
	if(ret != 1){
		hi846_awb_valid = 0;
		LOG_INF("=== hi846_data_awb invalid ===\n");
	}else{
		hi846_awb_valid = 1;
	}
	...
	if(ret != 1){
		hi846_lsc_valid = 0;
		LOG_INF("=== hi846_data_lsc invalid ===\n");
	}else{
		hi846_lsc_valid = 1;
	}
	ret = read_hi846_sn_info();
	if(ret != 1){
		hi846_sn_valid = 0;
		LOG_INF("=== hi846_data_sn invalid ===\n");
	}else{
		hi846_sn_valid = 1;
	}
	...
	if(hi846_module_id == 0 || hi846_lsc_valid == 0 || hi846_awb_valid == 0 || hi846_sn_valid == 0){
		return 0;
	}else{
		return 1;
	}
...
kal_uint16 addr_data_pair_preview_hi846[] = {
	...
	*/
	...
	//MIPI speed(Mbps): 688Mbps x 4Lane
	//Frame Length	  : 2526
	//Line Length	  : 3800
...
kal_uint16 addr_data_pair_capture_fps_hi846[] = {
	...
	*/
	...
	0x002C, 0x09CF,
	0x005C, 0x2101,
	0x0006, 0x13BC, // frame_length_lines_h
	0x0008, 0x0ED8, // line_length_pck_h
	0x000C, 0x0022,
	0x0A22, 0x0000,
	0x0A24, 0x0000,
...
//Sensor Information////////////////////////////
//Sensor          : Hi-846
//Image size      : 2448x1836
//MCLK/PCLK       : 24MHz /288Mhz
//MIPI speed(Mbps): 688Mbps x 4Lane
//Frame Length    : 3156
//Line Length     : 3800
//Max Fps         : 24.01fps(vblank>17.4ms)
//Firmware Ver.   : v10.5.1
////////////////////////////////////////////////
0x002E, 0x1111,
0x0032, 0x1111,
...
0x002C, 0x089D,
0x005C, 0x2101,
0x0006, 0x0C54, // frame_length_lines_h
0x0008, 0x0ED8, // line_length_pck_h
0x000C, 0x0022,
0x0A22, 0x0000,
0x0A24, 0x0000,
...
//Frame Length    : 3156
//Line Length     : 3800
//Max Fps         : 24.01fps(vblank>29.5ms)
//Firmware Ver.   : v10.5.1
////////////////////////////////////////////////
0x002E, 0x1111,
0x0032, 0x1111,
...
write_cmos_sensor(0x002E, 0x1111);
write_cmos_sensor(0x0032, 0x1111);
write_cmos_sensor(0x0026, 0x02A4);
write_cmos_sensor(0x002C, 0x076B);
write_cmos_sensor(0x005C, 0x2101);
...
write_cmos_sensor(0x0A22, 0x0000);
write_cmos_sensor(0x0A24, 0x0000);
write_cmos_sensor(0x0804, 0x0330);
write_cmos_sensor(0x0A12, 0x0660);
write_cmos_sensor(0x0A14, 0x04C8);
write_cmos_sensor(0x0074, 0x09D8);
write_cmos_sensor(0x021C, 0x0001);
write_cmos_sensor(0x0A04, 0x014A);
write_cmos_sensor(0x0418, 0x047C);
...
write_cmos_sensor(0x2008, 0x38FD);
write_cmos_sensor(0x326E, 0x0000);
write_cmos_sensor(0x0710, 0x04E8);
write_cmos_sensor(0x0900, 0x0300);
write_cmos_sensor(0x0902, 0xC319);
write_cmos_sensor(0x0914, 0xC109);
write_cmos_sensor(0x0916, 0x061A);
...
static void read_cmos_eeprom_table(kal_uint16 addr, kal_uint8 *table, kal_uint32 size)
{
	...
	iReadRegI2C(pusendcmd, 2, (u8 *)table, size, imgsensor.i2c_write_id);
}
...
static kal_uint32 set_test_pattern_mode(kal_uint8 modes, struct SET_SENSOR_PATTERN_SOLID_COLOR *pTestpatterndata)
{
	...
	test_pattern_save = (read_cmos_sensor(0x0A04) << 8) | read_cmos_sensor(0x0A05);
...
// 0x5E00[8]: 1 enable,  0 disable
// 0x5E00[1:0]; 00 Color bar, 01 Random Data, 10 Square, 11 BLACK
		write_cmos_sensor(0x0A04, test_pattern_save);
