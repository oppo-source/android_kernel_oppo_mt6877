...
static DEFINE_MUTEX(gimx882pd_eeprom_mutex);

static bool selective_read_eeprom(kal_uint16 addr, BYTE *data)
{
    char pu_send_cmd[2] = { (char)(addr >> 8), (char)(addr & 0xFF) };

    if (addr > IMX882PD_MAX_OFFSET)
        return false;

    if (iReadRegI2C(pu_send_cmd, 2, (u8 *) data,
            1, IMX882PD_EEPROM_SLAVE_ADDRESS) < 0) {
        return false;
    }
    return true;
}

static bool read_imx882pd_eeprom(kal_uint16 addr, BYTE *data, int size)
{
    int i = 0;
    int offset = addr;
...
unsigned int read_imx882pd_23231_SPC(kal_uint16 *data)
{
    kal_uint16 idx = 0, sensor_startL_reg = 0xD200, sensor_startR_reg = 0xD300;
    static BYTE imx882pd_SPC_data[SPC_SIZE] = { 0 };
    ...
    mutex_lock(&gimx882pd_eeprom_mutex);
    if ((readed_size == 0) &&
        read_imx882pd_eeprom(pinfo->SPC_addr,
            imx882pd_SPC_data, pinfo->SPC_size)) {
        readed_size = pinfo->SPC_size;
    }
    mutex_unlock(&gimx882pd_eeprom_mutex);
    ...
    for (idx = 0; idx < SPC_SIZE; idx++) {
        ...
        data[2 * idx + 1] = imx882pd_SPC_data[idx];
    }
    ...
    read_imx882pd_eeprom(0x162E, &SPC_flag, 1);
    pr_info("SPC flag0x%x[1:valid, other:Invalid]", SPC_flag);
    return readed_size;
}

unsigned int read_imx882pd_23231_QSC(kal_uint16 * data)
{
    kal_uint16 idx = 0, sensor_qsc_address = 0xC000;
    kal_uint8 tmp_QSC_setting[QSC_SIZE];
    kal_uint8 qsc_ver = 0;

    read_imx882pd_eeprom(OTP_QSC_OFFSET, tmp_QSC_setting, QSC_SIZE);
    ...
    for (idx = 0; idx < QSC_SIZE; idx++) {
        pr_debug("alex qsc data imx882pd_QSC_setting[0x%x] = 0x%x",
            data[2 * idx], data[2 * idx + 1]);
    }

    read_imx882pd_eeprom(0x2A32, &qsc_ver, 1);
