...
int aw37004_camera_power_up(int out_iotype)
{
	...
	case OUT_DVDD1:
		...
		ret = aw37004_i2c_write(camera_chip, aw37004_on_config[OUT_DVDD1].reg, 0x64);//bit0
		if (ret < 0) {
	...
	case OUT_DVDD2:
		...
		mutex_lock(&aw37004_mutex);
		ret = aw37004_i2c_write(camera_chip, aw37004_on_config[OUT_DVDD2].reg, 0x4B);//bit1
		if (ret < 0) {
			mutex_unlock(&aw37004_mutex);
	...
	case OUT_AVDD2:
		...
		mutex_lock(&aw37004_mutex);
		ret = aw37004_i2c_write(camera_chip, aw37004_on_config[OUT_AVDD2].reg, 0x78);//bit3
		if (ret < 0) {
			mutex_unlock(&aw37004_mutex);
			pr_err("AW37004 set avdd1 failed\n");
			break;
