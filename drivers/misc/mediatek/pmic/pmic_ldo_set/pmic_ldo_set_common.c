...
extern int DIO8018_ldo6_get_enable_reg(void);
extern int DIO8018_ldo6_get_value_reg(void);
#endif
...
#ifdef OPLUS_FEATURE_DISPLAY
	int (*ldo6_get_enable_reg)(void);
	int (*ldo6_get_value_reg)(void);
#endif
};
...
#ifdef OPLUS_FEATURE_DISPLAY
				ldo_ops.ldo6_get_enable_reg = DIO8018_ldo6_get_enable_reg;
				ldo_ops.ldo6_get_value_reg = DIO8018_ldo6_get_value_reg;
#endif
				return 0;
			} else {
...
#ifdef OPLUS_FEATURE_DISPLAY
int pmic_dio_get_ldo_6_enable_reg()
{
	if(ldo_type != LDO_UNKNOW) {
		return ldo_ops.ldo6_get_enable_reg();
	} else {
		if(!pmic_ldo_get_type()) {
			return ldo_ops.ldo6_get_enable_reg();
		}
...
EXPORT_SYMBOL(pmic_dio_get_ldo_6_enable_reg);

int pmic_dio_get_ldo_6_value_reg()
{
	if(ldo_type != LDO_UNKNOW) {
		return ldo_ops.ldo6_get_value_reg();
	} else {
		if(!pmic_ldo_get_type()) {
			return ldo_ops.ldo6_get_value_reg();
		}
...
EXPORT_SYMBOL(pmic_dio_get_ldo_6_value_reg);
#endif
