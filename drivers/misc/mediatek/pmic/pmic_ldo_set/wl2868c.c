...
/* #ifdef OPLUS_FEATURE_DISPLAY */
	if (oplus_check_status == 1 && reg ==0x0E && val == 0x80) {
		pr_err("%s:oplus_check_status %d write reg 0x%x val 0x%x ok\n", __func__, oplus_check_status, reg, val);
		return 0;
	}
/* #endif */
