...
static void mtk_nanohub_init_sensor_info(void)
{
	...
	if (is_project(0x2169E) || is_project(0x2169F) || is_project(0x216C9) || is_project(0x216CA) || is_project(20827) || is_project(20831) || is_project(21881) || is_project(21882)) {
		p->gain = 100000;
	} else {
		p->gain = 1000000;
	}
...
int mtk_nanohub_cfg_to_hub(uint8_t sensor_id, uint8_t *data, uint8_t count)
{
	...
	memcpy(cmd->data, data, count);
        pr_err("mtk_nanohub cfgdata %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d\n",
		cmd->data[0], cmd->data[1], cmd->data[2], cmd->data[3],
		cmd->data[4], cmd->data[5], cmd->data[6], cmd->data[7],
		cmd->data[8], cmd->data[9], cmd->data[10], cmd->data[11],
		cmd->data[12], cmd->data[13], cmd->data[14], cmd->data[15]);
	if (atomic_read(&power_status) == SENSOR_POWER_UP) {
		ret = nanohub_external_write((const uint8_t *)cmd,
	...
	if (data->flush_action == DATA_ACTION) {
		...
		default: // CCT and PWM_RGB run this branch to report data
			...
			if (debug_count_data == 0) {
				pr_err("mtk_nanohub_report_to_manager: sensor_type:%d, data[0]:%d, data[1]:%d, data[2]:%d, data[3]:%d, data[4]:%d, data[5]:%d, data[6]:%d, data[7]:%d\n", (int)event.sensor_type, (int)data->data[0],(int)data->data[1],(int)data->data[2],(int)data->data[3],(int)data->data[4],(int)data->data[5], (int)data->data[6], (int)data->data[7]);
			}
			debug_count_data++;
			break;
		}
