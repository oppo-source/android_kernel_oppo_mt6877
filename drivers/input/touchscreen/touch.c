...
bool __init tp_judge_ic_match(char *tp_ic_name)
{
	...
	case 22366:
	case 22367:
		pr_info("[TP] tp judge ic for 2235x & 2236x\n");
		if (strstr(tp_ic_name, "ilitek,ili7807s") && strstr(saved_command_line, "oplus22351_csot_ili7807s_fhd_dsi_vdo")) {
			return true;
		}
		if (strstr(tp_ic_name, "ilitek,ili7807s") && strstr(saved_command_line, "oplus22361_boe_ili9883c_hd_dphy_vdo_lcm")) {
			return true;
		}
		if (strstr(tp_ic_name, "novatek,nf_nt36672c") && strstr(saved_command_line, "oplus22351_tm_nt36672c_fhd_dsi_vdo")) {
			return true;
		}
		pr_err("[TP] ERROR! ic is not match driver\n");
		return false;
...
bool  tp_judge_ic_match_commandline(struct panel_info *panel_data)
{
	...
	case 22366:
	case 22367:
		if (strstr(saved_command_line, "oplus22351_csot_ili7807s_fhd_dsi_vdo")) {
			pr_err("[TP] touch ic = ilt7807S_cost \n");
			tp_used_index = ili7807s_csot;
			g_tp_dev_vendor = TP_HUAXING;
		}

		if (strstr(saved_command_line, "oplus22351_tm_nt36672c_fhd_dsi_vdo")) {
			pr_err("[TP] touch ic = nt36672c_tm \n");
			tp_used_index = nt36672c_tm;
			g_tp_dev_vendor = TP_TIANMA;
		}

		if (strstr(saved_command_line, "oplus22361_boe_ili9883c_hd_dphy_vdo")) {
...
int tp_util_get_vendor(struct hw_resource *hw_res, struct panel_info *panel_data)
{
	...
	/* Fanli */
	if (prj_id == 22087 || prj_id == 22088 || prj_id == 22331 || prj_id == 22332 || prj_id == 22333 || prj_id == 21334) {
